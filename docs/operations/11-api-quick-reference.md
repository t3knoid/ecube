...
**Pause Recovery After Restart:** `POST /jobs/{job_id}/pause` moves the job into `PAUSING` immediately and relies on in-flight workers to finish their current file work before the normal transition to `PAUSED`. If the service restarts after the pause request but before that final status write, startup reconciliation restores the job to `PAUSED`, clears the stranded active-copy marker, and resets any stale `COPYING` or `RETRYING` file rows back to `PENDING` so the same job can resume cleanly.
