...
| 1h | Dashboard needs-attention section highlights follow-up work | Seed failed, paused, and pending jobs plus a completed or archived mount whose trusted custody state is `PENDING_HANDOFF`, then open Dashboard as a processor or manager | The Dashboard shows a `Needs Attention` section with rows labeled `Blocked`, `Waiting to Start`, and `Waiting on Custody Closeout`, and each `Job ID` value routes safely into the matching Job Detail page |
| 1i | Dashboard job rows show derived next-step guidance | Seed pending jobs that are both ready and still analyzing, running jobs, failed or paused jobs, and a completed or archived job that still has pending custody handoff, then open Dashboard as a processor or manager | The `Needs Attention` and active-jobs tables show a read-only `Next Step` column with guidance such as start, wait for analysis, monitor progress, review failed files, resume, or review verification and handoff based only on trusted job and custody state; unknown or unsupported states fall back to opening Job Detail |
