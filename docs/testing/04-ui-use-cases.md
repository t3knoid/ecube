...
**UI Implication:** Mount list with status badges (MOUNTED/UNMOUNTED/ERROR). Add-mount form with protocol selector (NFS vs SMB), remote path input, and conditional credential fields. Local mount points are generated by the backend and displayed after mount creation. Validate button per mount and a "Validate All" bulk action.
