...
| `JobsView.vue` | Screen 6a | UC-6.1, UC-6.3 | Job list with status/progress plus the grouped shared create-job dialog that filters mounts/drives by project, keeps source-path browsing inside the trusted directory browser, and pins dialog header/footer while the body scrolls |
| `JobDetailView.vue` | Screen 6b–d | UC-6.2 – UC-7.15 | Progress bar with polling; edit/pause/complete/delete lifecycle actions; grouped shared edit dialog with notes support, trusted source-path browsing, and pinned header/footer scrolling; gated verify/manifest controls; hash viewer; source/destination compare; manifest success feedback; job-scoped CoC dialog that loads stored snapshots, shows a `Generated At` timestamp sourced from stored snapshot metadata, supports explicit refresh for admin/manager, and exposes print/export/handoff controls according to role |
...
|-----------|---------|--------|
| `StatusBadge.vue` | Colored pill showing state (DISCONNECTED, DISABLED, AVAILABLE, IN_USE, PREPARING, RUNNING, etc.) | DrivesView, JobsView, MountsView |
...
| `JobEditorDialogContent.vue` | Shared grouped job create/edit dialog shell with required-field legend, trusted source-path browser integration, and internal-body scrolling | JobsView, JobDetailView |
