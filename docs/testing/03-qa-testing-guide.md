...
| 9 | Formatted drives clear stale evidence UI state | Format one of the drives from the scenario above, then stay on Drive Detail and return to Drives | Once formatting clears the project binding, Drive Detail shows `Evidence` as `-`, the Drives row no longer shows the old evidence value, and stale project or evidence links are not shown for that drive |
...
| 12 | Device and serial columns stay aligned across UI | Open Drives, Jobs, and Create/Edit Job for the same connected drive | Drives shows `Device` plus `Serial Number`; Jobs list shows the same `Device` value; destination selector is labeled `Select device` and uses the same port-based label |
