...
| Create dialog seeds copy tuning fields from the live configuration defaults | Open the create dialog and switch to `Workflow` | `Thread count`, `Copy Chunk Size`, `Progress Flush Threshold`, and `Force per-file disk sync` are pre-selected to the values currently shown on `Configuration` > `Copy and Job Workflow` (matching settings such as `COPY_DEFAULT_THREAD_COUNT`, `COPY_CHUNK_SIZE_BYTES`, `COPY_PROGRESS_FLUSH_BYTES`, and `COPY_FILE_FSYNC_ENABLED`) |
| Create dialog tracks updated configuration defaults | Change one or more copy-tuning values on `Configuration` > `Copy and Job Workflow` (or update the matching `.env` value and restart), then reopen the create dialog and switch to `Workflow` | The pre-selected values on the create dialog match the new configuration values without manual refresh of the Jobs page; jobs that were already created keep their original snapshotted values and do not pick up the new defaults retroactively |
