...
class UsbDriveSchema(BaseModel):
    ...
    device_identifier: str = Field(..., description="Stable device identifier (USB serial number when available, otherwise sysfs device path)")
    filesystem_path: Optional[str] = Field(default=None, description="OS device node path for the block device (e.g., /dev/sda); may be null if not available")
