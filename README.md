...
- **Database migrations** — ECUBE uses a release-scoped Alembic workflow. During an unreleased app cycle, update the single current release migration in `alembic/versions` instead of creating a second unreleased revision file. Use `ecube-release-migration ensure|create|autogenerate` as documented in [docs/development/00-development-guide.md](docs/development/00-development-guide.md#database-and-migrations).
- **In-app help generation** — the packaged Help modal is generated from [docs/operations/13-user-manual.md](docs/operations/13-user-manual.md) by `node scripts/build-help.mjs`, which writes [frontend/public/help/manual.html](frontend/public/help/manual.html). Regenerate the HTML locally when the manual changes, then rely on the repo pre-commit hook, `npm run build`, and CI to verify the checked-in help file is already in sync before packaging artifacts are produced.
