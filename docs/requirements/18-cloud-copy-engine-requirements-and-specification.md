...
|---|---|---|
| NFR-001 | Performance | The engine **SHALL** sustain aggregate write throughput of at least 80% of the theoretical USB bus bandwidth when writing to a single device with a fast cloud source. For USB 3.0 devices, the target is >=2,400 Mb/s sustained sequential write throughput (device hardware permitting). |
