...
| `CALLBACK_ALLOW_PRIVATE_IPS` | `false` | Allow HTTPS callbacks to private/reserved IP addresses. Keep this `false` in production so HTTPS callbacks stay pinned to publicly routable destinations. Test-only `http://` callbacks skip the private-address SSRF gate and should remain limited to lab workflows. |
