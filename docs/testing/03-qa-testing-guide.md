...
| 16 | Files table uses `View Hashes` without checksum listing | Expand the Files panel for a completed job and inspect the table headers and row actions | The table does not show a `Checksum` column, and authorized users have `View Hashes` available for per-file hash inspection |
