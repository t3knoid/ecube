...
| 17f | Large-drive format does not fail from the prior browser request timeout | Start formatting a large exFAT-capable USB drive from Drive Detail and keep the page open longer than 30 seconds | The browser does not show the old generic network error solely because the request exceeded 30 seconds; if the backend times out, the operator instead sees the backend failure path and can adjust configuration |
