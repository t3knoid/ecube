...
By default, `GET /drives` returns `DISABLED`, `AVAILABLE`, and `IN_USE` drives, and excludes disconnected (`DISCONNECTED`) rows. To include `DISCONNECTED` drives, add `include_disconnected=true`:
...
To update an existing share definition in place, use the mount `id` returned above:
...
{
  "type": "NFS",
  "remote_path": "fileserver:/exports/case-2026-001-updated",
  "project_id": "CASE-2026-001"
}
```
...
- If exactly one `AVAILABLE` drive is bound to the project, it is selected.
  If the drive is temporarily unavailable (e.g. locked by a concurrent
...
Operators can configure the callback URL from the Jobs create dialog and the Job Detail edit dialog. API clients can also continue to supply `callback_url` directly when creating or updating a job. Administrators can also set a system-wide `CALLBACK_DEFAULT_URL` from the Admin page; that default is used only when the job itself does not define `callback_url`.
