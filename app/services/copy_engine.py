...
def _log_copy_job_runtime_parameters(
    ...
    *,
    ...
    thread_count_source: Optional[str] = None,
    copy_chunk_size_source: Optional[str] = None,
    copy_progress_flush_source: Optional[str] = None,
    copy_file_fsync_source: Optional[str] = None,
) -> None:
    ...
    logger.debug(
        ...
        extra={
            ...
            "thread_count_source": thread_count_source or tuning.thread_count_source,
            ...
            "copy_chunk_size_source": copy_chunk_size_source or tuning.copy_chunk_size_source,
...
def _read_copy_job_snapshot(db: Session, job_id: int) -> Optional[ExportJob]:
    read_db = Session(bind=db.get_bind())
    try:
        latest_job = JobRepository(read_db).get(job_id)
        if latest_job is None:
            return None
        read_db.expunge(latest_job)
        return latest_job
    finally:
        read_db.close()


def _list_pending_copy_job_files(
    db: Session,
    job_id: int,
    *,
    ...
    read_db = Session(bind=db.get_bind())
    try:
        pending_files = FileRepository(read_db).list_pending_by_job_after_id(
            job_id,
            after_id=after_id,
            limit=limit,
        )
        for pending_file in pending_files:
            read_db.expunge(pending_file)
        return pending_files
    finally:
        read_db.close()


def _count_copy_job_file_states(
    db: Session,
    job_id: int,
    *,
    ...
    try:
        return int(
            read_db.query(ExportFile)
            .filter(
                ExportFile.job_id == job_id,
                ExportFile.status.in_(statuses),
            )
            .count()
            or 0
        )
...
def _count_copy_job_done_and_errors(db: Session, job_id: int) -> tuple[int, int]:
    read_db = Session(bind=db.get_bind())
    try:
        return FileRepository(read_db).count_done_and_errors(job_id)
    finally:
        read_db.close()
...
def _read_copy_job_runtime_tuning_snapshot(db: Session, job_id: int) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        if latest_job is None:
            return None
        tuning = resolve_job_copy_tuning(latest_job)
        return {
            "job": latest_job,
            "max_workers": int(tuning.effective_thread_count),
            "thread_count_source": tuning.thread_count_source,
            "copy_chunk_size_bytes": int(tuning.effective_copy_chunk_size_bytes),
            "copy_chunk_size_source": tuning.copy_chunk_size_source,
            "copy_progress_flush_threshold_bytes": int(resolve_progress_flush_threshold_bytes(latest_job)),
            "copy_progress_flush_source": tuning.copy_progress_flush_source,
            "copy_file_fsync_enabled": bool(tuning.effective_copy_file_fsync_enabled),
...
class _CopyProgressWriter:
    """Serialize copy-progress counter updates through a single DB session."""
    ...
    def __init__(
        ...
        *,
        db: Session,
        job_id: int,
        flush_interval_seconds: float = COPY_PROGRESS_DB_FLUSH_INTERVAL_SECONDS,
    ) -> None:
    ...
    def record_delta(self, assignment_id: Optional[int], delta_bytes: int) -> None:
        if delta_bytes == 0:
            return
        if not self._background_enabled:
            with self._pending_lock:
                self._pending_job_delta += int(delta_bytes)
        ...
        *,
        ...
        try:
            repo.apply_copy_progress_batch(
                self._job_id,
                job_delta_bytes=pending_job_delta,
                assignment_delta_bytes={
            ...
            logger.exception("DB commit failed flushing aggregated copy progress", extra={"job_id": self._job_id})
            return False
    ...
    def _run(self) -> None:
        ...
        try:
            while True:
                ...
                while True:
                    try:
                        queued_item = self._updates.get_nowait()
                    except queue.Empty:
                        break
...
def run_copy_job(job_id: int) -> None:
    ...
    **Resume semantics**: if the job already has ``DONE`` export-file records
    ...
    try:
        ...
        try:
            while True:
                job_snapshot = _read_copy_job_snapshot(db, job_id)
                if job_snapshot is None:
                    return

                source = Path(job_snapshot.source_path)
                target = Path(job_snapshot.target_mount_path) if job_snapshot.target_mount_path else None
                ...
                if job_snapshot.status in (JobStatus.PAUSING, JobStatus.PAUSED):
                    ...
                    logger.info(
                        ...
                        extra={
                            "job_id": job_id,
                            "project_id": job_snapshot.project_id,
                ...
                max_retries = (
                    job_snapshot.max_file_retries
                    if job_snapshot.max_file_retries is not None
                    else settings.copy_default_max_retries
                )
                retry_delay = (
                    float(job_snapshot.retry_delay_seconds)
                    if job_snapshot.retry_delay_seconds is not None
                    else settings.copy_default_retry_delay_seconds
                )

                if not pause_requested:
                    stale_run_detected = False
                    active_tuning_key: Optional[tuple[int, int, int, bool]] = None
                    active_tuning_sources: Optional[tuple[str, str, str, str]] = None
                    control_poll_count = 0
                    control_poll_reason_counts: dict[str, int] = {}
                    completions_since_control_poll = 0
                    max_completions_between_polls = 0
                    max_refill_latency_seconds = 0.0
                    refill_pending_since: Optional[float] = None
                    last_control_poll_monotonic = time.monotonic()
                    max_workers = int(resolve_job_copy_tuning(job_snapshot).effective_thread_count)
                    copy_chunk_size_bytes = int(resolve_job_copy_tuning(job_snapshot).effective_copy_chunk_size_bytes)
                    copy_progress_flush_threshold_bytes = int(resolve_progress_flush_threshold_bytes(job_snapshot))
                    copy_file_fsync_enabled = bool(resolve_job_copy_tuning(job_snapshot).effective_copy_file_fsync_enabled)
                    ...
                    def _poll_runtime_control_state(reason: str, cancel_pending: list[Any] | None = None) -> bool:
                        nonlocal control_poll_count
                        nonlocal completions_since_control_poll
                        nonlocal last_control_poll_monotonic
                        nonlocal max_completions_between_polls
                        nonlocal active_tuning_key
                        nonlocal active_tuning_sources
                        nonlocal max_workers
                        nonlocal copy_chunk_size_bytes
                        nonlocal copy_progress_flush_threshold_bytes
                        nonlocal copy_file_fsync_enabled

                        now = time.monotonic()
                        poll_interval_seconds = max(0.0, now - last_control_poll_monotonic)
                        control_poll_count += 1
                        control_poll_reason_counts[reason] = control_poll_reason_counts.get(reason, 0) + 1
                        max_completions_between_polls = max(
                            max_completions_between_polls,
                            completions_since_control_poll,
                        )
                        metrics_service.record_copy_scheduler_control_poll(
                            reason=reason,
                            interval_seconds=poll_interval_seconds,
                            completions_since_last=completions_since_control_poll,
                        )
                        last_control_poll_monotonic = now
                        completions_since_control_poll = 0
                    ...
                    progress_writer = _CopyProgressWriter(db=db, job_id=job_id)
                    ...
                    try:
                        ...
                        def _load_pending_buffer(limit: int) -> None:
                            ...
                            if pending_exhausted:
                                return
                            pending_files = _list_pending_copy_job_files(
                                db,
                                job_id,
                        ...
                        def _submit_file_attempt(file_id: int, relative_path: str, *, attempt: int) -> None:
                            ...
                            if refill_pending_since is not None:
                                ...
                                max_refill_latency_seconds = max(max_refill_latency_seconds, refill_latency_seconds)
                                metrics_service.record_copy_scheduler_refill_latency(latency_seconds=refill_latency_seconds)
                        ...
                        while True:
                            ...
                            poll_due_to_completion_budget = (
                                completions_since_control_poll >= COPY_CONTROL_POLL_COMPLETION_INTERVAL
                            )
                            poll_due_to_interval = (
                                (time.monotonic() - last_control_poll_monotonic)
                                >= COPY_CONTROL_POLL_MAX_INTERVAL_SECONDS
                            )
                            if poll_due_to_completion_budget or poll_due_to_interval:
                                poll_reason = "completion_budget" if poll_due_to_completion_budget else "interval"
                                if _poll_runtime_control_state(poll_reason, list(futures)):
                                    if stale_run_detected:
                                        return
                                    if pause_requested:
                                        break
                        ...
                        logger.debug(
                            "Copy scheduler control polling summary",
                            extra={
                                "job_id": job_id,
                                "poll_count": control_poll_count,
                                "poll_completion_interval_limit": COPY_CONTROL_POLL_COMPLETION_INTERVAL,
                                "poll_max_interval_seconds_limit": COPY_CONTROL_POLL_MAX_INTERVAL_SECONDS,
                                "max_completions_between_polls": max_completions_between_polls,
                                "max_refill_latency_seconds": round(max_refill_latency_seconds, 6),
                                "poll_reason_counts": control_poll_reason_counts,
                            },
                        )
                ...
                if latest_status is not None and latest_started_at_key != run_started_at_key:
                    ...
                    return
                lingering_copy_state_count = _count_copy_job_file_states(
                    db,
                    job_id,
                    statuses=(FileStatus.COPYING, FileStatus.RETRYING),
                )
                if lingering_copy_state_count > 0 and not pause_requested:
                    ...
                    try:
                        ...
                        return
                    continue
                done_count, error_count = _count_copy_job_done_and_errors(db, job_id)
                timeout_count = _count_copy_job_file_states(
                    db,
                    job_id,
                    statuses=(FileStatus.TIMEOUT,),
                )
