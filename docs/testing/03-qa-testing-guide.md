...
| Empty `DEMO_SHARED_PASSWORD` follows the active password policy | Enable demo mode with `DEMO_SHARED_PASSWORD` unset, note the login screen shared password, tighten the Password Policy, refresh the login page, then restart ECUBE | The refreshed login page shows a new generated shared password that matches the updated policy, and after restart the displayed password works for demo account sign-in |
...
| 8 | Runtime default demo password follows Password Policy when unset | Enable demo mode with `DEMO_SHARED_PASSWORD` unset, verify the login screen password, change Password Policy settings, restart ECUBE, then retry demo sign-in with the newly displayed password | The shared password shown on the login page changes to match the updated policy, and the restarted service accepts that password for the demo accounts |
