...
describe('JobDetailView start action', () => {
  ...
  it('keeps the files panel collapsed by default and pages through file rows with a 5-page window', async () => {
    ...
    expect(wrapper.find('.columns-stub').text()).toContain(`${i18n.global.t('jobs.path')}|${i18n.global.t('common.labels.status')}`)
    expect(wrapper.find('.columns-stub').text()).not.toContain(i18n.global.t('common.labels.size'))
    expect(wrapper.find('.columns-stub').text()).not.toContain(i18n.global.t('jobs.destinationDrive'))
    expect(wrapper.text()).not.toContain('1.5 KB')
