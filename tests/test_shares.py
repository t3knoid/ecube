...
def test_list_shares_includes_related_job_custody_status_for_completed_jobs(client, db, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.services.share_service.get_job_custody_summaries",
...
def test_update_share_updates_existing_record_without_creating_new_one(manager_client, db):
    ...
    db.expire_all()
    mounts = db.query(NetworkShare).order_by(NetworkShare.id).all()
    assert len(mounts) == 1
...
def test_update_share_preserves_existing_credentials_when_not_resubmitted(manager_client, db):
    with patch("app.services.share_service._ensure_mount_directory", return_value=None), \
        ...
        original = db.query(NetworkShare).filter(NetworkShare.id == share_id).one()
...
def test_update_share_uses_stored_credentials_when_not_resubmitted(db):
    ...
    with patch("app.services.share_service.decrypt_mount_secret", side_effect=["svc-reader", "super-secret", None]), \
         patch("app.services.share_service._ensure_mount_directory", return_value=None), \
         patch("app.services.share_service._validate_mount_directory_owner", return_value=None):
...
def test_validate_share_uses_stored_credentials_when_share_is_not_active(db):
    ...
    with patch("app.services.share_service.decrypt_mount_secret", side_effect=["svc-reader", "super-secret", None]), \
         patch("app.services.share_service.check_mounted_with_configured_timeout", return_value=False), \
         patch("app.services.share_service._ensure_mount_directory", return_value=None), \
         patch("app.services.share_service._validate_mount_directory_owner", return_value=None):
        result = share_service.validate_share(mount.id, db, provider=provider)
...
def test_add_share_fails_when_mountpoint_owned_by_root(manager_client, db):
    with patch("app.services.share_service._ensure_mount_directory", return_value=None), \
         patch(
             "app.services.share_service._validate_mount_directory_owner",
...
def test_delete_share(manager_client, db):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0)
        response = manager_client.delete(f"/shares/{share_id}")
...
def test_cleanup_generated_mount_directory_does_not_use_sudo(monkeypatch):
    ...
    with patch("app.services.share_service.os.rmdir", side_effect=PermissionError("denied")), \
         patch("subprocess.run") as mock_run:
...
def test_ensure_mount_directory_uses_sudo_mkdir_and_chown_for_managed_paths(monkeypatch):
    monkeypatch.setattr("app.services.share_service.settings.use_sudo", True)
    monkeypatch.setattr("app.services.share_service.os.geteuid", lambda: 1000)
    monkeypatch.setattr("app.services.share_service.os.getegid", lambda: 1000)

    with patch("app.services.share_service.os.makedirs", side_effect=PermissionError("denied")), \
         patch("app.services.share_service.pwd.getpwuid") as mock_getpwuid, \
         patch("app.services.share_service.grp.getgrgid") as mock_getgrgid, \
         patch("subprocess.run") as mock_run:
...
def test_delete_unmounted_mount_skips_os_unmount_and_removes_record(manager_client, db):
    ...
    provider = MagicMock()
    provider.check_mounted.return_value = False
...
def test_delete_share_unmounts_when_live_mount_state_disagrees_with_record(manager_client, db):
    mount = NetworkShare(
        type=MountType.NFS,
        remote_path="192.168.1.1:/share",
        local_mount_point="/mnt/share",
    ...
    provider = MagicMock()
    provider.check_mounted.return_value = True
...
def test_validate_share_success(manager_client, db):
    ...
    with patch("subprocess.run") as mock_run:
        mock_run.return_value = MagicMock(returncode=0)
        response = manager_client.post(f"/shares/{share_id}/validate")
...
def test_validate_share_logs_unmount_failure_for_mounted_share(manager_client, db, caplog):
    mount = NetworkShare(
        type=MountType.NFS,
        remote_path="192.168.1.1:/data",
        project_id="PROJ-VAL",
    ...
    provider = FailingUnmountProvider()

    with caplog.at_level(logging.DEBUG, logger="app.services.share_service"):
        with patch("app.services.share_service._default_provider", return_value=provider):
            response = manager_client.post(
                f"/shares/{mount.id}/validate",
                json={
                    "type": "NFS",
                    "remote_path": "192.168.1.1:/data",
                    "project_id": "proj-val",
    ...
    assert info_record.context["error_code"] == "MOUNT_UNMOUNT_FAILED"
    assert info_record.context["failure_class"] == "mount_validate_unmount_failed"
    assert info_record.context["trace_id"] == trace_id
    ...
    assert debug_record.context["error_code"] == "MOUNT_UNMOUNT_FAILED"
    assert debug_record.context["trace_id"] == trace_id
    assert debug_record.context["raw_error"] == "umount: /mnt/data: target is busy"


def test_validate_share_logs_restore_failure_for_mounted_share(manager_client, db, caplog):
    mount = NetworkShare(
        type=MountType.NFS,
        remote_path="192.168.1.1:/data",
        project_id="PROJ-VAL",
    ...
    provider = RestoreFailingProvider()
...
def test_validate_share_candidate_warns_when_nfs_41_is_slow_and_nfs_3_is_faster(manager_client, db):
    ...
    with patch("app.services.share_service._default_provider", return_value=provider), \
         patch("app.services.share_service._ensure_mount_directory", return_value=None), \
         patch("app.services.share_service._validate_mount_directory_owner", return_value=None), \
         patch("app.services.share_service._timed_mount_attempt", side_effect=fake_timed_mount_attempt):
        response = manager_client.post(
            "/shares/test",
            json={
                "type": "NFS",
                "remote_path": "192.168.20.240:/volume1/demo-case-002",
                "project_id": "proj-slow-nfs41",
                "nfs_client_version": "4.1",
            },
        )
...
def test_validate_share_candidate_reports_nfs_3_fallback_when_nfs_41_times_out(manager_client, db):
    ...
    def fake_timed_mount_attempt(_provider, _mount_type, _remote_path, _local_mount_point, **kwargs):
        ...
        if kwargs.get("nfs_client_version") == "4.1":
            ...
            return False, "Operation timed out", 30.0
        provider.mounted = True
    ...
    with patch("app.services.share_service._default_provider", return_value=provider), \
        ...
        response = manager_client.post(
            ...
            json={
                "type": "NFS",
                "remote_path": "192.168.20.240:/volume1/demo-case-002",
                "project_id": "proj-timeout-fallback",
                "nfs_client_version": "4.1",
            },
        )
...
def test_discover_share_shares_allows_admin_role(admin_client, db):
    ...
    with patch("app.services.share_service._default_provider", return_value=FakeProvider()):
...
def test_discover_share_shares_allowed_in_demo_mode(manager_client, db):
    ...
    with patch.object(type(settings), "is_demo_mode_enabled", return_value=True), \
            patch("app.services.share_service._default_provider", return_value=FakeProvider()):
        response = manager_client.post(
            "/shares/discover",
            json={
                "type": "NFS",
...
def test_discover_share_shares_returns_actionable_message_when_host_tool_is_missing(manager_client, db):
    ...
    with patch("app.services.share_service._default_provider", return_value=MissingToolProvider()):
        response = manager_client.post(
            "/shares/discover",
            json={
                "type": "SMB",
...
def test_validate_share_not_found(manager_client, db):
    response = manager_client.post("/shares/999/validate")
    assert response.status_code == 404
...
def test_linux_mount_provider_discover_shares_uses_dedicated_timeout():
    ...
    with patch("app.services.share_service._resolve_showmount_binary", return_value="/usr/sbin/showmount"), \
         patch("subprocess.run") as mock_run:
...
def test_linux_mount_provider_smb_discover_shares_uses_dedicated_timeout():
    ...
    with patch("app.services.share_service._resolve_smbclient_binary", return_value="/usr/bin/smbclient"), \
         patch("subprocess.run") as mock_run:
...
def test_linux_mount_provider_unmount_uses_plain_sudo_umount(monkeypatch):
    provider = LinuxShareProvider()
    ...
    assert cmd == ["sudo", "-n", settings.umount_binary_path, "/mnt/music"]
...
def test_linux_mount_provider_uses_plain_sudo_mount(monkeypatch):
    provider = LinuxShareProvider()
    ...
    assert err is None
    cmd = mock_run.call_args_list[0].args[0]
    assert cmd[:3] == ["sudo", "-n", settings.mount_binary_path]
    assert "-N" not in cmd
...
def test_linux_mount_provider_returns_retry_error_when_fstab_retry_fails():
    ...
    with patch("app.services.share_service._resolve_mount_nfs_binary", return_value=None), \
...
def test_linux_mount_provider_uses_direct_nfs_helper_after_fstab_failures(caplog):
    ...
    assert not any("command_path=" in message for message in info_messages)
    assert any(
...
def test_linux_mount_provider_info_logs_do_not_expose_command_paths(caplog):
    provider = LinuxShareProvider()
    ...
    assert err is None
    info_messages = [record.getMessage() for record in caplog.records if record.levelname == "INFO"]
    assert any("Executing NFS mount command" in message for message in info_messages)
    assert any("Mount command succeeded" in message for message in info_messages)
    assert not any("command_path=" in message for message in info_messages)
