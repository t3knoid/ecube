...
`export_jobs` now also stores startup-analysis lifecycle and summary fields: `startup_analysis_status`, `startup_analysis_last_analyzed_at`, `startup_analysis_failure_reason`, `startup_analysis_file_count`, `startup_analysis_total_bytes`, `startup_analysis_share_read_mbps`, `startup_analysis_drive_write_mbps`, `startup_analysis_estimated_duration_seconds`, and `startup_analysis_entries`. The `startup_analysis_*_mbps` fields are bit-based throughput measurements in `Mb/s`, not byte-based `MB/s` values. It also stores `copy_started_at`, which records when the current active copy phase entered `RUNNING` so the API and UI can separate total job duration from copy-only rate and ETA calculations. In PostgreSQL, `startup_analysis_entries` is stored as `JSONB` and is cleared when the reusable startup-analysis snapshot is invalidated, explicitly removed, or automatically discarded after successful completion, while summary fields can remain available for operator review and API responses.
...
stateDiagram-v2
  [*] --> EMPTY
  DISCONNECTED --> DISABLED: Discovery detects drive on disabled port
  DISCONNECTED --> AVAILABLE: Discovery detects present drive on enabled port
  DISABLED --> AVAILABLE: Port enabled and discovery reruns
  AVAILABLE --> IN_USE: Initialize with project_id
  IN_USE --> AVAILABLE: Prepare-eject
  AVAILABLE --> DISABLED: Port disabled while drive still present
