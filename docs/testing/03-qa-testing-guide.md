...
| Configuration exposes the global copy tuning defaults | Sign in as `manager` or `admin`, open `Configuration`, and inspect `Copy and Job Workflow` | The page shows `Copy Chunk Size`, `Progress Flush Threshold`, `Default Copy Worker Count`, and `Force per-file disk sync` as the global defaults that jobs inherit when no per-job override is stored |
