...
| Mount table parsing uses the configured Linux proc mounts path | Run the backend mount-info test file and inspect the mount-path assertions | Tests verify mount-table parsing reads the configured `procfs_mounts_path` directly so mount discovery stays aligned with the active runtime configuration |
