...
| POST | `/drives/{drive_id}/format` | admin/manager | Submit an asynchronous drive format for `ext4` or `exfat`; drive must be AVAILABLE and unmounted. Returns `202 Accepted`, sets drive `format_status` to `PENDING`, and clears the state when formatting completes or records a sanitized failure on the drive record |
