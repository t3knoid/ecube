...
| Archived jobs leave drive/share related-job context but stay in dashboard follow-up | Archive a job with incomplete custody or a recorded handoff while its destination drive is still mounted, then open `Drives`, `Drive Detail`, `Shares`, `Share Detail`, and `Dashboard` | Drives and shares show `No related job` for the archived work, while `Dashboard` keeps the archived job in `Needs Attention` with `Waiting on Custody Closeout` or `Ready to Eject` based on trusted job custody state |
...
| 1e | Mixed archived snapshot failure degrades only the affected share | Seed two completed or archived shares in the same response, then make one archived related job return a malformed or unreadable stored chain-of-custody snapshot while the other still has valid custody data | The affected share contributes to the Dashboard `Unavailable` count, while unrelated shares keep their correct `Completed` or `Active` classification; the whole shares response does not collapse to unknown custody state |
...
| 1ga | Shares workflow filter supports custody-pending drill-down | Open Dashboard with at least one completed or archived share whose custody status is `PENDING_HANDOFF`, activate the `Custody Pending` summary entry, and inspect the Shares page | ECUBE routes to Shares with the `Custody Pending` workflow bucket preselected and only rows in that authoritative bucket remain visible |
| 1h | Dashboard needs-attention section highlights follow-up work | Seed failed, paused, and pending jobs plus completed or archived mounts whose trusted custody state is both `PENDING_HANDOFF` and `HANDOFF_RECORDED` while the related drive remains mounted or `IN_USE`, then open Dashboard as a processor or manager | The Dashboard shows a `Needs Attention` section with rows labeled `Blocked`, `Waiting to Start`, `Waiting on Custody Closeout`, and `Ready to Eject`, and each `Job ID` value routes safely into the matching Job Detail page |
| 1i | Dashboard job rows show derived next-step guidance | Seed pending jobs that are both ready and still analyzing, running jobs, failed or paused jobs, a completed or archived job with pending custody handoff, and a completed or archived job with recorded custody handoff whose drive remains mounted or `IN_USE`, then open Dashboard as a processor or manager | The `Needs Attention` and active-jobs tables show a read-only `Next Step` column with guidance such as start, wait for analysis, monitor progress, review failed files, resume, review verification and handoff, or prepare eject based only on trusted job, custody, and drive mount state; unknown or unsupported states fall back to opening Job Detail |
