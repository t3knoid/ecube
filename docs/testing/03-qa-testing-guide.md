...
| 7 | Job ID links to the assigned job | Initialize or seed two drives so they share a project but have different assigned jobs and evidence numbers, then open Drives | Each row shows `Job ID`, the Filesystem, Project, and Evidence columns are absent, and selecting the job ID opens the matching Job Detail for that drive's assignment rather than another drive on the same project |
