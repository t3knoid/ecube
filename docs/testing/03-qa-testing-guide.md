...
| 6 | Per-file timeout failure is visible | Configure or simulate a timed-out file copy attempt, then open Job Detail after the job reaches `FAILED` | The failure context includes a per-file timeout message such as `File copy timed out after 3600s` instead of a whole-job timeout summary |
