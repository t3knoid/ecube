...
class PublicAuthConfigResponse(BaseModel):
    ...
    network_mount_timeout_seconds: int = Field(default=120, description="Default network mount timeout in seconds for public-safe UI workflows")
    mount_share_discovery_timeout_seconds: int = Field(default=60, description="Default mount share discovery timeout in seconds for public-safe UI workflows")
...
def public_auth_config() -> PublicAuthConfigResponse:
    ...
    public-safe UI defaults such as NFS client version hints and mount timeout
    values. In demo mode it may intentionally expose the shared demo password
    so unattended users can sign in.
    """
