...
- If the service restarts while the job is still in `PAUSING`, the next startup reconciliation restores the job to `PAUSED` and returns any stale `COPYING` or `RETRYING` file rows to `PENDING` before the operator resumes the job.
