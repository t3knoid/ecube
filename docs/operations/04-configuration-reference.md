...
| `DEMO_SHARED_PASSWORD` | generated from active password policy | Shared demo password exposed by the public auth payload and used by the login form for self-serve demo sign-in while demo mode is enabled. When this variable is left empty, ECUBE derives a policy-friendly default from the active Password Policy settings. Demo reconciliation applies this password to configured demo OS accounts when demo mode is enabled, so the effective value must satisfy the active PAM password policy when host password-policy enforcement is enabled. The setup-completion request skips an immediate redundant reset only for the setup-managed demo account it just authenticated, while still reconciling the remaining configured demo users. |
...
| `MKFS_EXFAT_CLUSTER_SIZE` | `4K` | Allocation unit ECUBE passes to `mkfs.exfat` when formatting USB drives as exFAT. Admins can change this from the Configuration page. `4K` is recommended for most situations. Use larger values only for drives that will store very large files only and no small files. Supported values are `4K`, `64K`, `128K`, `256K`. |
| `DRIVE_FORMAT_TIMEOUT_SECONDS` | `900` | Maximum seconds ECUBE allows a drive format command to run before it is treated as failed. Admins can change this from the Configuration page. Increase this for large removable media that legitimately require extended format time. |
| `DRIVE_MOUNT_TIMEOUT_SECONDS` | `120` | Maximum seconds ECUBE allows a drive mount command to run before it is treated as failed. Admins can change this from the Configuration page. Increase this for large removable media or slower USB links that need extra time to mount. |
| `NETWORK_MOUNT_TIMEOUT_SECONDS` | `120` | Maximum seconds ECUBE allows SMB or NFS mount workflows to run before they are treated as failed. This setting also applies to related unmount and mount-state verification checks. Admins can change this from the Configuration page. Increase this for slow file servers, DNS resolution, or authentication handshakes. |
| `MOUNT_SHARE_DISCOVERY_TIMEOUT_SECONDS` | `60` | Maximum seconds ECUBE allows SMB share or NFS export discovery to run before it is treated as failed. Admins can change this from the Configuration page. Increase this for slow file servers or networks. |
...
| `USB_DISCOVERY_INTERVAL`   | `0`    | Runtime USB discovery mode selector. `0` turns off timed polling and turns on event-driven udev monitoring mode. Positive values turn off udev runtime monitoring and run timed polling at that interval in seconds.            |
...
For NFS shares, ECUBE persists a per-share `nfs_client_version` only when an operator selects an explicit override in the Mounts dialog. When a share is left on the default option, mount creation, validation, and startup reconciliation use the current global `NFS_CLIENT_VERSION` instead.
...
| `CALLBACK_ALLOW_PRIVATE_IPS` | `false` | Allow HTTP and HTTPS callbacks to private/reserved IP addresses for controlled lab environments. Keep this `false` in production so callback delivery stays pinned to publicly routable destinations. |
...
| `CALLBACK_DEFAULT_URL`       | unset   | Optional callback URL used when a job does not define its own `callback_url`. HTTPS is the normal production path. `http://` is accepted only for testing when the admin explicitly confirms insecure callback use in the UI or API request. A job-specific callback URL always overrides the system-wide default. |
...
| `STARTUP_ANALYSIS_SMALL_FILE_MAX_BYTES` | `65536` | Fresh-install default for the maximum file size counted in the startup-analysis small-file bucket for workload-profile recommendation. The Configuration page shows the current saved value, which can differ from this default. Files larger than the current small threshold and smaller than `STARTUP_ANALYSIS_LARGE_FILE_MIN_BYTES` count as medium. |
| `STARTUP_ANALYSIS_LARGE_FILE_MIN_BYTES` | `8388608` | Fresh-install default for the minimum file size counted in the startup-analysis large-file bucket for workload-profile recommendation. The Configuration page shows the current saved value, which can differ from this default. Files between the current small and large thresholds count as medium. |
...
| `PWQUALITY_CONF_PATH`        | `/etc/security/pwquality.conf` | Path to the PAM password-quality configuration file used by the password-policy API. Native installs seed this on the host; the Docker image seeds the same baseline inside the container during build. |
| `PASSWORD_POLICY_WRITER_PATH` | `/usr/local/bin/ecube-write-pwquality-conf` | Root-owned helper used for atomic writes to `pwquality.conf` when `USE_SUDO=true`. Docker deployments run as root by default, so container-local writes use `PWQUALITY_CONF_PATH` directly instead of this helper. |
