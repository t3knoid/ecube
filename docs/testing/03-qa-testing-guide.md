...
| 9 | Demo setup succeeds when the setup account is already the demo admin | Enable demo mode, complete setup through the wizard using the configured demo admin account, then inspect `/etc/passwd`, `/etc/group`, and the login page | Setup completes without the generic demo-seeding failure banner, the remaining configured demo users are created, and the login page shows the shared demo credentials without requiring a restart or retry |
