...
def test_initialize_drive(client, db):
    drive = UsbDrive(device_identifier="USB002", current_state=DriveState.AVAILABLE)
    db.add(drive)
    db.commit()

    response = client.post(f"/drives/{drive.id}/initialize", json={"project_id": "PROJ-001"})
    assert response.status_code == 200
    data = response.json()
...
def test_initialize_drive_not_found(client, db):
    response = client.post("/drives/999/initialize", json={"project_id": "PROJ-001"})
    assert response.status_code == 404


def test_project_isolation_violation(client, db):
    drive = UsbDrive(
    ...
    response = client.post(f"/drives/{drive.id}/initialize", json={"project_id": "PROJ-002"})
    assert response.status_code == 409


def test_reinitialize_same_project(client, db):
    drive = UsbDrive(
...
def test_prepare_eject(client, db):
    drive = UsbDrive(
...
def test_prepare_eject_not_found(client, db):
    response = client.post("/drives/999/prepare-eject")
    assert response.status_code == 404
