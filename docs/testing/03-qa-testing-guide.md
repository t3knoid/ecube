...
| 1j | Dashboard job rows surface trusted operational context | Seed an active job with a related mounted share, a destination drive assignment, and live transfer timing plus a blocked job with failed and timed-out file counts, then open Dashboard as a processor or manager | The Dashboard row metadata shows the trusted source mount and source path, destination drive label, failure counters for blocked work, and live-copy rate, time remaining, and estimated completion values when the backend fields are available; missing drive or mount context falls back to `N/A` instead of inventing values |
