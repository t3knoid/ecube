...
| 17ea | Admin or manager can tune automatic USB discovery polling | Open `Configuration` as `admin` or `manager`, change `Auto USB Discovery Interval (seconds)` to `0`, save, wait longer than the prior interval, then set it back to a positive value | The field persists after save, automatic USB discovery pauses while the value is `0`, and periodic polling resumes after a positive interval is restored without requiring a service restart |
