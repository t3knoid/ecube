...
| 19 | View drive detail after initialization | Drive Detail shows the safe device label, total capacity, and last known available space when the drive is mounted; it does not display a `Filesystem Path` row, exposes `Browse`, and does not expose raw internal identifiers or host mount paths |
