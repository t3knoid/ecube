...
- On mobile Mounts rows, the visible mount ID remains the path into Mount Detail, while the visible `Project` value opens browse directly for mounted shares instead of relying on any overflow action.
...
| 20 | View the mounts list actions and browse controls | Raw remote and local mount paths are redacted in the table; the list uses `Mount ID` for detail navigation, uses the mounted row `Project` value to open browse directly, titles the panel as `Browse mount <project> contents`, starts mounted-share browsing at `/`, avoids a leading `//` breadcrumb, and does not expose `Edit` or `Remove` |
