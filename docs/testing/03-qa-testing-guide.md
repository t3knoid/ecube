...
| Archived Job Detail is read-only for lifecycle actions | Open an archived job in Job Detail | The job remains viewable, Edit and Analyze stay visible but disabled, the lifecycle toggle is hidden, and no Archive action is shown |
...
- When the dialog opens, keyboard focus moves to the `Project` field, `Tab` stays trapped inside the modal, `Escape` closes the dialog, and focus returns to the `Create Job` trigger.
