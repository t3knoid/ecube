...
| Configuration exposes startup-analysis auto-apply control | Sign in as `manager` or `admin`, open `Configuration`, and inspect `Copy and Job Workflow` | The page shows `Auto-apply startup analysis profile`, and save operations persist the setting through the manager configuration API |
...
| Auto-apply recommendation does not override explicit per-job tuning | Enable auto-apply in `Configuration`, create or edit a job with explicit per-job copy tuning overrides, run startup analysis, then inspect Job Detail | The recommendation is still shown, but the existing per-job override values remain unchanged until an operator explicitly applies the recommended profile |
