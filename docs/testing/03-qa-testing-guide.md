...
| 5e | Job detail actions emit lifecycle callbacks | Use a `webhook.site` HTTPS URL, manually complete a paused job, generate a manifest, refresh chain of custody, confirm custody handoff, and archive the job | `webhook.site` receives `JOB_COMPLETED_MANUALLY`, `MANIFEST_CREATED`, `COC_SNAPSHOT_STORED`, `COC_HANDOFF_CONFIRMED`, and `JOB_ARCHIVED` callbacks as each action persists |
