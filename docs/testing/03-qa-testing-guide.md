...
- On mobile Mounts rows, the visible mount ID remains the path into Mount Detail, while the visible `Project` value opens browse directly for mounted shares without requiring any overflow action.
...
| 20 | View the mounts list actions and browse controls | Raw remote and local mount paths are redacted in the table; the list uses `Mount ID` for detail navigation, uses the mounted row `Project` value to open browse directly, titles the panel as `Browse mount <project> contents`, starts mounted-share browsing at `/`, avoids a leading `//` breadcrumb, and does not expose `Edit`, `Remove`, or a separate `Browse` row action |
| 20a | View mount detail and manage the mount from that page | Open Mount Detail from the Mounts list | Mount Detail shows Type, Project, NFS Client version, Last Checked, Latest Read Speed, Last Throughput Test, Job ID, Job Status, and mount Status for all permitted viewers; when a related job exists the `Job ID` stays actionable and `Job Status` comes from trusted backend job data, while mounts with no related job show `No related job` and mounts with unavailable authoritative status show `Status unavailable`. Raw Remote Path and Local Mount Point values remain visible only to admin and manager users, while read-only roles see those path fields redacted. `Browse` is available for mounted shares, and `Edit`/`Remove` remain available only for admin and manager users |
| 20c | Run mounted-share throughput test from Mount Detail | Open Mount Detail for a mounted share as `admin` or `manager`, click `Test Throughput`, then refresh the page | The action completes without exposing raw share paths, `Latest Read Speed` and `Last Throughput Test` update, and the same stored values remain visible after refresh; `processor` and `auditor` do not see the action |
| 20b | Mounts list and Mount Detail keep related job context aligned | Open a mount row that shows a related `Job ID`, note the visible job ID, then open Mount Detail for that same mount | The Mounts row and Mount Detail show the same related `Job ID`, the Mount Detail `Job Status` describes that same related job, and the UI does not show conflicting related-job context between the list and detail surfaces |
...
| 11 | Edit an active NFS mount with a new per-share client version | Update the mount through `PATCH /mounts/{mount_id}` or the Mount Detail `Edit` dialog while the share is `MOUNTED` | 200, the mount remains `MOUNTED`, the returned `nfs_client_version` matches the edited override, and the live share is remounted with the new option |
...
| 1c | Dashboard mounts summary reflects operator decision buckets | Seed mounts with no related job, a not-yet-started related job (`PENDING`), active jobs (`RUNNING`, `PAUSING`, or `VERIFYING`), blocked jobs (`PAUSED` or `FAILED`), terminal jobs whose custody is both pending and recorded, and mounts whose related-job context is unavailable, then open Dashboard as an operational user | The Dashboard shows a `Mounts Summary` card alongside `Drive Summary`, with `Unassigned`, `Assigned`, `Active`, `Blocked`, `Custody Pending`, `Completed`, and `Unavailable` counts that match the authoritative related-job and custody data returned by the backend; `Assigned` only counts not-yet-started jobs, `Active` counts in-flight work, `Blocked` counts paused or failed work, completed or archived jobs stay `Custody Pending` until custody handoff is recorded, and unavailable related-job context is counted explicitly instead of being omitted |
| 1d | Auditor dashboard hides operational dashboard sections | Sign in as an `auditor` user and open Dashboard | The Dashboard still shows the system-health summary, but it does not render the `Mounts Summary`, `Drive Summary`, `Needs Attention`, or active-jobs panels |
| 1e | Mixed archived snapshot failure degrades only the affected mount | Seed two completed or archived mounts in the same response, then make one archived related job return a malformed or unreadable stored chain-of-custody snapshot while the other still has valid custody data | The affected mount contributes to the Dashboard `Unavailable` count, while unrelated mounts keep their correct `Completed` or `Active` classification; the whole mounts response does not collapse to unknown custody state |
| 1f | Dashboard auto-refresh updates the full operational snapshot | Open Dashboard with at least one active job, then let job, drive, mount, or needs-attention state change without using the manual refresh button | Within about 10 seconds, the Dashboard refreshes system health, drive summary, mount summary, needs-attention content, and active-jobs content together so the cards and tables reflect the latest backend state |
| 1g | Dashboard summary drill-down preserves operational context | Open Dashboard as an operational user and activate a `Drive Summary` or `Mounts Summary` entry with mouse or keyboard | ECUBE routes to the matching Drives or Mounts page, and the destination filter control shows the corresponding drive state or mount workflow bucket selected |
| 1ga | Mounts workflow filter supports custody-pending drill-down | Open Dashboard with at least one completed or archived mount whose custody status is `PENDING_HANDOFF`, activate the `Custody Pending` summary entry, and inspect the Mounts page | ECUBE routes to Mounts with the `Custody Pending` workflow bucket preselected and only rows in that authoritative bucket remain visible |
