...
4. Prepare the managed mount roots used by ECUBE (the configured network mount base path, default `/mnt/ecube-network`, and the configured USB mount base path, default `/mnt/ecube`) with service-account ownership and runtime-safe permissions. On first install, set `NETWORK_MOUNT_BASE_PATH` and/or `USB_MOUNT_BASE_PATH` in the installer environment if you need non-default managed mount roots so the generated `.env` and prepared directories stay aligned. Keep the network root as a separate absolute path that does not overlap the USB root.
...
6. Install `/etc/sudoers.d/ecube-user-mgmt` with narrowly scoped `NOPASSWD` rules for setup OS user/group management, `chage`, password-policy helper writes, mount/unmount operations, selected drive filesystem commands, and explicit admin-triggered runtime repair commands such as `modprobe`.
...
This mode enables `DEMO_MODE=true`, writes the runtime `DEMO_*` values derived from `demo-metadata.json` into `.env`, ensures the application database is ready, and seeds demo accounts from `demo-metadata.json` located beside `install.sh`. After the appliance setup wizard completes, ECUBE also reruns runtime demo account seeding so newly initialized systems do not need a restart before demo role assignments are present. When the setup wizard itself authenticates with the configured demo admin account, that post-setup reconciliation still creates the remaining demo users but skips an immediate redundant password reset for the just-configured setup account so PAM password-history or unchanged-password rules do not block setup completion. On later service starts, runtime startup reconciliation keeps those demo OS users, DB roles, and shared-password state aligned with the installed `DEMO_*` settings. If `demo_config.shared_password` is blank, the installer generates and prints a strong demo password, stores it in the installed metadata, and writes it into `.env` so demo sign-in can be prefilled and managed through the public auth payload. It does not generate demo metadata, discover USB devices, stage sample content, or create a separate `demo-data` directory.
