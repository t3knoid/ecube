...
| 1d | Auditor dashboard hides operational mount summary | Sign in as an `auditor` user and open Dashboard | The Dashboard still shows the system-health summary, but it does not render the `Mounts Summary`, `Drive Summary`, or active-jobs panels |
...
| 1f | Dashboard auto-refresh updates the full operational snapshot | Open Dashboard with at least one active job, then let job, drive, or mount state change without using the manual refresh button | Within about 10 seconds, the Dashboard refreshes system health, drive summary, mount summary, and active-jobs content together so the cards and table reflect the latest backend state |
