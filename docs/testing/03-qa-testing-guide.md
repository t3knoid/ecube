...
| 20a | View mount detail and manage the mount from that page | Open Mount Detail from the Mounts list | Mount Detail shows Type, Remote Path, Project, NFS Client version, Local Mount Point, Last Checked, Job ID, and Status; `Browse` is available for mounted shares, and `Edit`/`Remove` remain available only for admin and manager users |
