...
| 1e | Mixed archived snapshot failure degrades only the affected mount | Seed two completed or archived mounts in the same response, then make one archived related job return a malformed or unreadable stored chain-of-custody snapshot while the other still has valid custody data | The affected mount contributes to the Dashboard `Unavailable` count, while unrelated mounts keep their correct `Completed` or `In Progress` classification; the whole mounts response does not collapse to unknown custody state |
