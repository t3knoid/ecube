...
| Docker Compose USB mount, unmount, and reconciliation use the documented container profile | Start the stack with `docker compose -f docker-compose.ecube.yml up -d --build`, connect a test USB drive to the host, open `Drives` in the ECUBE UI, mount the drive into its managed ECUBE slot, confirm browse access, then unmount it from ECUBE and trigger or wait for the next reconciliation refresh | The `ecube-app` container detects the drive through the documented USB, udev, and sysfs mounts, ECUBE mounts the drive successfully, browse remains available only while the managed mount is active, unmount succeeds cleanly, and reconciliation returns the drive to the correct post-unmount state without depending on any extra host procfs bind mount |

## Manual Verification Additions for Job Source Share Validation

Add the following manual checks when a release includes source-share selection or share-backed job create validation changes.

| Scenario | Steps | Expected |
|---|---|---|
| Job create surfaces share terminology for source-share validation failures | Open `Jobs`, start the create dialog, select a source share that is unavailable to the chosen project or no longer mounted, then attempt to save | The job is not created, and the validation message refers to the selected `share` consistently rather than `mount`, matching the current Shares workflow language |
...
| Project-bound rediscovered drive keeps ownership without trusting an unmanaged host mount | Initialize a drive to a project, leave it host-mounted outside the managed ECUBE USB root, restart or refresh discovery, then open `Drives` and Drive Detail | The drive keeps its project-bound state, but no browse entry point is restored from the unmanaged host mount and operator workflows that require a managed mounted drive remain unavailable until ECUBE mounts the drive into its managed slot |
...
| Archived jobs leave drive/share related-job context and dashboard follow-up | Archive a job with incomplete custody or a recorded handoff while its destination drive is still mounted, then open `Drives`, `Drive Detail`, `Shares`, `Share Detail`, and `Dashboard` | Drives and shares show `No related job` for the archived work, and `Dashboard` does not keep the archived job in `Needs Attention` |
...
| Restarting a failed job preserves successful copies | Open a `FAILED` job with mixed per-file outcomes, note `Files failed` and `Files timed out`, then click `Start` | The same job returns to `PREPARING` and then `RUNNING`; file rows already in `DONE` stay preserved, non-`DONE` rows are re-queued, and this is distinct from `Retry Failed Files` on partial-success `COMPLETED` jobs |
...
| Configuration tabs switch between manager setting groups | Sign in as `manager`, open `Configuration`, and use the tab strip with click or arrow-key navigation | The page opens on `Troubleshooting`, switches between the five manager setting groups without leaving the form, and keeps the save/cancel action row available throughout |
| Admin tabs switch between admin setting groups | Sign in as `admin`, open `Admin`, and use the tab strip with click or arrow-key navigation | The page opens on `Logging Infrastructure`, switches between the six admin setting groups without leaving the form, and keeps the save/cancel action row available throughout |
| Configuration exposes the global copy tuning defaults | Sign in as `manager` or `admin`, open `Configuration`, and inspect `Copy and Job Workflow` | The page shows `Startup Analysis Small-File Max Bytes`, `Startup Analysis Large-File Min Bytes`, `Copy Chunk Size`, `Progress Flush Threshold`, `Default Copy Worker Count`, `Force per-file disk sync`, and `Enable separate hashing thread` as the persisted defaults and copy-engine controls exposed on the Configuration page |
| Configuration rejects inverted startup-analysis thresholds | Sign in as `manager` or `admin`, open `Configuration`, set `Startup Analysis Small-File Max Bytes` to a value greater than or equal to `Startup Analysis Large-File Min Bytes`, then save | The save is rejected, the existing values remain unchanged, and the validation error explains that the small-file threshold must stay below the large-file threshold |
| Configuration validates a single startup-analysis threshold edit against the saved peer value | Sign in as `manager` or `admin`, open `Configuration`, change only one of the startup-analysis threshold fields while the other field is already saved to a non-default value, then save | The save is validated against the current saved peer threshold, not the fresh-install default or a process-local override, and the validation result matches the saved pair shown on the page |
| Create dialog exposes startup-analysis auto-apply control | Sign in as `manager` or `processor`, open `Jobs`, click `Create Job`, and switch to `Workflow` | The dialog shows `Auto-apply startup analysis profile` as a per-job checkbox and keeps it disabled until a project is selected |
| Create and edit dialogs can save per-job copy tuning overrides | Open `Jobs` or `Job Detail`, switch to the `Workflow` tab, set `Thread count`, `Copy Chunk Size`, `Progress Flush Threshold`, or `Force per-file disk sync` to a non-default value, save, then reopen the same job | The save succeeds, the chosen overrides persist on the job, and Job Detail shows the effective values together with whether each setting comes from the job override or the configuration default |
| Effective progress flush threshold matches runtime normalization | Create or edit a job with `Copy Chunk Size` larger than `Progress Flush Threshold`, save it, open Job Detail, and start the job | Job Detail shows the effective progress flush threshold clamped to at least the chosen chunk size, and the runtime behavior matches that displayed effective value |
| Job Detail shows startup-analysis recommendation and distribution | Open a job with completed startup analysis in Job Detail | `Source Information` includes `Recommended workload profile` and file-size distribution percentages for small, medium, and large files |
| Job Detail filters file rows by status | Open Job Detail, expand `Files`, select a file status filter such as `Error` or `Done`, then switch between filters | The file list refreshes to only rows with the selected file status, pagination restarts at the first page for the new filter, and clearing the filter returns to the full file list |
| Apply recommended profile updates per-job tuning | Open a non-active job with a startup-analysis recommendation in Job Detail, click `Apply Recommended Profile`, then reopen `Edit` > `Workflow` | The action succeeds, per-job copy tuning fields match the recommended profile preset, and the profile can be applied without changing source or destination definition |
| Auto-apply recommendation remains job-scoped and does not override explicit per-job tuning | Create a job with explicit per-job copy tuning overrides and also enable `Auto-apply startup analysis profile` in the same create dialog, run startup analysis, then inspect Job Detail | The recommendation is still shown, but existing per-job override values remain unchanged until an operator explicitly selects `Apply Recommended Profile` |
| Auto-apply recommendation runs when the job still uses the configured defaults | Create a job with the Workflow tab left at the configured defaults, enable `Auto-apply startup analysis profile`, run startup analysis, then inspect Job Detail | ECUBE applies the recommended workload profile after startup analysis completes and updates the stored per-job tuning values |
| Auto-apply recommendation uses the saved startup-analysis threshold buckets | Save non-default startup-analysis threshold values in `Configuration`, create a job that enables `Auto-apply startup analysis profile`, run startup analysis on files that cross the saved thresholds, then inspect Job Detail | The recommended profile and any auto-applied copy tuning match the saved threshold buckets from `Configuration`, even when the runtime process was started with different threshold environment values |
...
| Scenario | Steps | Expected |
|---|---|---|
| Pending job still supports full edit flow | Open a `PENDING` job in Job Detail and click `Edit` | The dialog opens on the `Details` tab, shows the same pre-start source and destination fields as create, and also exposes a `Workflow` tab for thread count, copy tuning, and reserved overflow selections |
| Create dialog seeds copy tuning fields from the live configuration defaults | Open `Configuration`, switch to `Copy and Job Workflow`, note the current values, then open the create dialog and switch to `Workflow` | `Thread count`, `Copy Chunk Size`, `Progress Flush Threshold`, and `Force per-file disk sync` are pre-selected to the values currently shown on the `Configuration` page `Copy and Job Workflow` tab (matching settings such as `COPY_DEFAULT_THREAD_COUNT`, `COPY_CHUNK_SIZE_BYTES`, `COPY_PROGRESS_FLUSH_BYTES`, and `COPY_FILE_FSYNC_ENABLED`) |
| Create dialog tracks updated configuration defaults | Change one or more copy-tuning values on the `Configuration` page `Copy and Job Workflow` tab (or update the matching `.env` value and restart), then reopen the create dialog and switch to `Workflow` | The pre-selected values on the create dialog match the new configuration values without manual refresh of the Jobs page; jobs that were already created keep their original snapshotted values and do not pick up the new defaults retroactively |
| Create dialog can override the configured copy-tuning defaults | Open the create dialog, switch to `Workflow`, change `Thread count`, `Copy Chunk Size`, `Progress Flush Threshold`, or `Force per-file disk sync` to a non-default value, save the job, and open Job Detail | The job is created successfully and Job Detail shows the explicit per-job override with source marked as the job |
| Saving the create dialog snapshots the configuration defaults onto the job | Note the current values on the `Configuration` page `Copy and Job Workflow` tab, open the create dialog, switch to `Workflow` without changing anything, save the job, then open Job Detail | All four copy-tuning fields on Job Detail show the configuration values that were live at create time, and the source for those values is reported as the job |
| Existing jobs do not retroactively follow new configuration defaults | After creating the job in the previous scenario, change at least one copy-tuning value on the `Configuration` page `Copy and Job Workflow` tab (or update the matching `.env` value and restart), then reopen Job Detail for that job | The previously-created job still shows the snapshotted values from create time, the new global default applies only to subsequently created jobs, and re-saving the existing job through `Edit` re-snapshots whatever values are currently shown on its `Workflow` tab |
| Copy-tuning defaults endpoint is restricted | Sign in as `auditor` and either call `GET /jobs/copy-tuning-defaults` directly or open the Job Editor through the UI | The API call returns `403 Forbidden`, no copy-tuning values are exposed, and the auditor session has no Job Editor entry point that surfaces the values |
| Started job opens restricted edit mode | Open a `RUNNING`, `PAUSED`, or `FAILED` job in Job Detail and click `Edit` | The dialog opens on `Workflow`, only copy tuning overrides and reserved overflow-drive selection remain editable, and the tab explains that project, source, primary destination, notes, and callback details stay fixed after the job starts |
| Started job can change copy tuning overrides only | Open a started job, change one or more copy tuning overrides, save, and refresh Job Detail | The save succeeds, the updated effective values are shown on the job, and no immutable job-definition fields change |
| Started-job tuning changes apply at safe runtime boundaries | Open a `RUNNING` job, change `Thread count`, `Copy Chunk Size`, `Progress Flush Threshold`, or `Force per-file disk sync`, save, and observe active copy behavior and progress continuity | The save succeeds immediately, currently in-flight file work is allowed to finish, and subsequent dispatched file work uses the updated tuning values without forcing a pause, cancellation, or job restart |
| Target-full continuation stops new dispatch before the next batch | Run a job with at least one reserved overflow drive, force a `target_full` failure, and keep Job Detail open while the active worker finishes | ECUBE finishes the in-flight file work, does not dispatch a new batch to the full destination, and then continues to the next eligible assigned drive or transitions to `FAILED` with the safe destination-capacity reason |
| Started job runtime tuning edit does not require an inferred source mount | Open a started job whose source path is no longer under a currently mounted project share, click `Edit`, change a copy tuning override, save, and refresh Job Detail | The save succeeds without reopening source selection, the runtime tuning change persists, and the restricted edit flow continues to treat source and mount identity as immutable job-definition fields |
...
| Job editor tabs remain keyboard operable | Open Create Job or Edit Job, use `Tab` to reach the tab strip, then use `ArrowLeft`, `ArrowRight`, `Home`, and `End` on the tab buttons | The active tab changes without leaving the dialog, the footer remains pinned, and the fields shown in the panel match the selected tab on desktop and smaller screens |
...
- In the same pending or preparing state, verify `Source Information` shows `Discovered files` and `Estimated total bytes` as `N/A` until trusted totals are available.
...
- After totals become available, verify `Source Information` updates `Discovered files` and `Estimated total bytes` to the trusted values.
...
- With multiple copy workers active and queued work remaining, trigger `Pause` just after one file finishes. ECUBE does not dispatch an additional queued file after the pause request, and the remaining pause delay comes only from the files that were already in flight.
...
- If the service restarts while the job is still in `PAUSING`, the next startup reconciliation restores unfinished work to `PAUSED` and returns any stale `COPYING` or `RETRYING` file rows to `PENDING`. If the copy run had already reached a terminal result before restart, the job stays terminal instead of reopening as paused work.
...
- Confirm `Job Information` shows `Job details`, `Source Information`, and `Destination Information` panels, with `Job details` first and populated from the job-creation values.
- Confirm `Source Information` includes the final lifecycle state plus discovered file count and estimated total bytes, and that failed runs show only a sanitized failure reason.
- Confirm `Destination Information` shows the selected drive serial number, vendor ID, and product ID when hardware discovery provides them, and that overflow assignments surface the same fields when present while still showing `None` when no overflow assignments are present.
...
- With a running job that hits a `target_full` copy failure and no other assigned drive, verify the job transitions to `FAILED`, shows only a sanitized destination-capacity reason, and records the failed file plus the job-level decision.
- With a running job that hits a `target_full` copy failure and already has another assigned overflow drive, verify ECUBE resumes automatically only when the next assigned drive stays project-compatible and passes the backend free-space probe; otherwise the job must stay `FAILED` until the operator chooses `Continue on Another Drive`.
...
- On the Shares page, verify the share ID value navigates to Share Detail, verify mounted rows use the `Project` value to open browsing directly for `admin`, `manager`, and `processor`, verify the browse panel title uses `Browse share <project> contents`, verify the mounted-share browser starts at `/`, verify the breadcrumb does not expose the share name or begin with `//`, verify the related `Job ID` navigates to Job Detail when present, and verify the list does not show a separate `Browse` action. For `auditor`, verify the same mounted row renders the project as read-only text and does not open browse.
...
- Drives, Shares, and Jobs hide lower-priority columns on smaller screens and keep the most important identifiers visible. Where list actions remain necessary, Shares and Jobs move them into an overflow menu instead of forcing horizontal page expansion.
...
- On mobile Shares rows, the visible share ID remains the path into Share Detail, while the visible `Project` value opens browse directly for mounted shares without requiring any overflow action.
- On mobile Dashboard, the summary cards stack vertically, `Needs Attention` and `Active Jobs` keep `Job ID` as the visible navigation column, both tables hide the `Project` column to avoid horizontal overflow, compact status icons still expose accessible labels or tooltips, Active Jobs shows the progress percentage without the wider-screen live-transfer block, and `Next Step` guidance remains visible.
- Mobile Jobs, Drives, and Shares rows render compact status indicators that still expose accessible labels or tooltips for the underlying state.
...
- On the System page, verify authenticated roles see only the supported tabs, and verify the `Logs` tab is visible to `admin` and `manager` users only.
- On the System page, verify the shared tab strip supports click and arrow-key navigation between `Health`, `USB Topology`, `Block Devices`, `Mounts`, and `Logs`, and verify the `Reconcile managed mounts` action is available from inside the `Mounts` tab for `admin` and `manager` only.
- Attach a non-storage USB device such as a keyboard, allow USB discovery to complete, and verify it does not appear in the Drives screen or in the `observed_drives` entries recorded by the latest `USB_DISCOVERY_SYNC` audit log.
...
- For a runtime repair action whose audit write fails, verify the API returns the audit-failure conflict response and the UI does not show a success banner for that action.
- On the Reconciliation Results page, the Current USB Drives and Current Shared Mounts tables follow the same compact mobile treatment as the main Drives and Shares pages.
...
- Uvicorn request traces such as `GET /jobs/1 200 OK` do not appear as `INFO` entries in the application log stream; operational `INFO` lines remain focused on ECUBE application and service events.
...
| Empty `DEMO_SHARED_PASSWORD` follows the active password policy | Enable demo mode with `DEMO_SHARED_PASSWORD` unset, sign in to a demo account, tighten the Password Policy, refresh the login page, then restart ECUBE | The refreshed login page still prefills the demo password field from the generated value that matches the updated policy, and after restart that effective password continues to work for demo account sign-in |
...
| 19 | View drive detail after initialization | Drive Detail shows the safe device label, total capacity, last known available space, latest stored write-throughput fields when available, and `Browse` when the drive is mounted; it does not display a `Filesystem Path` row and does not expose raw internal identifiers or host mount paths |
| 19a | Run mounted-drive throughput test from Drive Detail | Open Drive Detail for a mounted managed USB drive as `admin` or `manager`, click `Test Throughput`, then refresh the page | The action completes without exposing raw mount paths, `Latest Write Speed` and `Last Throughput Test` update, and the same stored values remain visible after refresh; `processor` and `auditor` do not see the action |
| 20 | View the shares list actions and browse controls | Raw remote and local mount paths are redacted in the table; the list uses `Share ID` for detail navigation, uses the mounted row `Project` value to open browse directly, titles the panel as `Browse share <project> contents`, starts mounted-share browsing at `/`, avoids a leading `//` breadcrumb, and does not expose `Edit`, `Remove`, or a separate `Browse` row action |
| 20a | View share detail and manage the share from that page | Open Share Detail from the Shares list | Share Detail shows Type, Project, NFS Client version, Last Checked, Latest Read Speed, Last Throughput Test, Job ID, Job Status, and share Status for all permitted viewers; when a related job exists the `Job ID` stays actionable and `Job Status` comes from trusted backend job data, while shares with no related job show `No related job` and shares with unavailable authoritative status show `Status unavailable`. Raw Remote Path and Local Mount Point values remain visible only to admin and manager users, while read-only roles see those path fields redacted. `Browse` is available for mounted shares, and `Edit`/`Remove` remain available only for admin and manager users |
| 20c | Run mounted-share throughput test from Share Detail | Open Share Detail for a mounted share as `admin` or `manager`, click `Test Throughput`, then refresh the page | The action completes without exposing raw share paths, `Latest Read Speed` and `Last Throughput Test` update, and the same stored values remain visible after refresh; `processor` and `auditor` do not see the action |
| 20b | Shares list and Share Detail keep related job context aligned | Open a share row that shows a related `Job ID`, note the visible job ID, then open Share Detail for that same share | The Shares row and Share Detail show the same related `Job ID`, the Share Detail `Job Status` describes that same related job, and archived jobs do not remain in either surface as the current related job after archive |
| 21 | Operate the Initialize and Add Share dialogs with keyboard only | Focus enters the dialog, Tab stays trapped within it, Escape closes it, and focus returns to the triggering control |
| 22 | Use `Browse` in the Add Share dialog with many discovered shares | The share picker opens, supports vertical scrolling, and selecting a share fills the `Remote path` field |
| 23 | Open Add Share while demo mode is enabled | The share-discovery `Browse` control remains available and opens the share picker normally |
...
| 11 | Drive on disabled port stays DISABLED | Plug in drive on disabled port, run `POST /drives/refresh` | `GET /drives` shows drive in `DISABLED` state |
...
| 14 | Disable port — AVAILABLE drive demoted | Enable port, confirm drive is `AVAILABLE`, disable port, run `POST /drives/refresh` | Drive transitions to `DISABLED`; `GET /drives` still returns it by default |
| 15 | Orphan drive stays DISABLED | Discover a drive with no matching port (`port_id = NULL`), run `POST /drives/refresh` | Drive remains in `DISABLED` state (unknown port treated as disabled) |
...
| 19 | Enable Drive available for physically detected disabled drive | Insert a drive on a disabled but known port, refresh, then open drive detail as admin or manager | The Enable Drive action is available for the `DISABLED` drive; after enable plus refresh the drive becomes `AVAILABLE` or remains `IN_USE` if already mounted |
...
| # | Test | How | Expected |
|---|------|-----|----------|
| 1 | Validate mounted NFS share | Create an NFS share, then `POST /shares/{share_id}/validate` | 200, share remains `MOUNTED`, `last_checked_at` advances |
| 2 | Validate mounted SMB share | Create an SMB share, then `POST /shares/{share_id}/validate` | 200, share remains `MOUNTED` |
| 3 | Validate disconnected share | Break connectivity or unmount externally, then `POST /shares/{share_id}/validate` | 200, share status becomes `UNMOUNTED` or `ERROR` with a descriptive error |
| 4 | Validate all shares reports mixed states | Register one reachable share and one broken share, then `POST /shares/validate` | 200, response includes updated status for each share |
| 5 | Validate missing share | `POST /shares/9999/validate` | 404, `NOT_FOUND` |
| 6 | Processor cannot validate shares | `POST /shares/{share_id}/validate` with processor token | 403, `FORBIDDEN` |
| 7 | Share validation audit trail | Validate a share, then query `GET /audit?action=MOUNT_VALIDATED` | Audit entry records actor, share id, and resulting status |
| 8 | Reject exact duplicate remote path | Create a share, then submit the same `remote_path` again for the same project | 409, `CONFLICT`, operator-readable message indicating the source is already configured |
...
| 10 | Allow nested path for same project | Create a parent source for project A, then add a child path for the same project | 200, second share is accepted |
| 11 | Edit an active NFS share with a new per-share client version | Update the share through `PATCH /shares/{share_id}` or the Share Detail `Edit` dialog while the share is `MOUNTED` | 200, the share remains `MOUNTED`, the returned `nfs_client_version` matches the edited override, and the live share is remounted with the new option |
| 11 | Normalize SMB path variants before duplicate check | Create an SMB share with slash-separated path, then submit the same source with backslash separators | 409, `CONFLICT`, duplicate source rejected |
| 12 | Reject concurrent duplicate submit | Submit two near-simultaneous add-share requests for the same remote source from separate sessions | Only one share is created; the competing request returns a safe conflict instead of producing duplicates |
| 13 | Preserve stored credentials on edit when fields are left blank | Edit an existing SMB share in the UI, change only remote path or project ID, and save without entering credential values | Update succeeds and the existing stored credentials remain in effect |
| 14 | Explicitly clear stored credentials during edit | Edit an existing SMB share in the UI, click `Clear saved credentials`, and save | Update request clears stored username/password/credentials file and no replacement credential values are required |
| 15 | Returned share error keeps edit dialog open | Edit a share so the backend returns an updated share object with `status: ERROR` | The dialog remains open and shows the returned failure state instead of showing a success-only close path |
| 16 | Shares list shows related Job ID for the mounted project | Seed or create a job for the same project as a listed share, then open Shares | The row shows `Project` and `Job ID`, selecting `Job ID` opens the matching Job Detail page, and rows without a valid related job keep the `Job ID` cell non-actionable |
...
| 6 | Drives list hides disconnected rows by default | Keep one historical `DISCONNECTED` drive and one connected drive, then open the Drives page without changing filters | The page defaults to `All`, shows `DISABLED`, `AVAILABLE`, and `IN_USE` drives only, and requires `Show Disconnected drives` before absent hardware appears |
...
| 8a | Project-bound drives without assignment show no job context | Initialize two drives to the same project, assign only one drive to a job, then open Drive Detail for the unassigned drive | Drive Detail keeps the shared `Project` value but shows `Evidence` and `Job ID` as `-`, `Custody Status` as `No related job`, and no job link for the unassigned drive |
| 8b | Explicit overflow assignments inherit the related job context | Initialize three drives to the same project, assign one as the primary destination, reserve or continue overflow onto one additional drive, then open Drive Detail for all three drives | The primary and explicitly assigned overflow drive each show the same related `Evidence` and `Job ID`, while the spare project-bound drive still shows no related job context |
| 9 | Formatted drives clear stale drive-job UI state | Format one of the drives from the scenario above, then stay on Drive Detail and return to Drives | Once formatting clears the project binding, Drive Detail shows `Evidence` and `Job ID` as `-`, the Drives row does not show the prior job ID value, and stale job links are not shown for that drive |
...
| 13 | Mounted drive surfaces capacity, available space, speed, throughput, and Browse entry | Open Drive Detail for a mounted managed USB drive | The Drives list does not show a Size column, Drive Detail shows total capacity, negotiated USB speed when available or `-` when it is unavailable, the latest stored write-throughput values when a prior test exists, plus the last known available space value or `-` when no reading is available yet, and the mounted drive exposes `Browse` plus manager/admin `Test Throughput` without exposing a raw mount-path link |
...
| 1a | Dashboard active job ID opens Job Detail | Start or seed an active job, open Dashboard, and select the `Job ID` value in the Active Jobs table | The `Job ID` cell is actionable for a valid active job and routes to the matching Job Detail page |
| 1b | Dashboard invalid job ID stays non-actionable | Simulate or seed an active-jobs row with a missing or invalid `id` value, then open Dashboard | The row shows a plain-text fallback instead of a broken button or link, and selecting the row does not attempt navigation |
| 1c | Dashboard shares summary reflects operator decision buckets | Seed shares with no related job, a not-yet-started related job (`PENDING`), active jobs (`RUNNING`, `PAUSING`, or `VERIFYING`), blocked jobs (`PAUSED` or `FAILED`), completed jobs whose custody is both pending and recorded, and shares whose related-job context is unavailable, then open Dashboard as an operational user | The Dashboard shows a `Shares Summary` card alongside `Drive Summary`, with `Unassigned`, `Assigned`, `Active`, `Blocked`, `Custody Pending`, `Completed`, and `Unavailable` counts that match the authoritative related-job and custody data returned by the backend; `Assigned` only counts not-yet-started jobs, `Active` counts in-flight work, `Blocked` counts paused or failed work, completed jobs stay `Custody Pending` until custody handoff is recorded, and unavailable related-job context is counted explicitly instead of being omitted |
| 1d | Auditor dashboard hides operational dashboard sections | Sign in as an `auditor` user and open Dashboard | The Dashboard still shows the system-health summary, but it does not render the `Shares Summary`, `Drive Summary`, `Needs Attention`, or active-jobs panels |
| 1e | Mixed related-job failure degrades only the affected share | Seed two completed shares in the same response, then make one related job return an unavailable custody classification while the other still has valid custody data | The affected share contributes to the Dashboard `Unavailable` count, while unrelated shares keep their correct `Completed` or `Active` classification; the whole shares response does not collapse to unknown custody state |
| 1f | Dashboard auto-refresh updates the full operational snapshot | Open Dashboard with at least one active job, then let job, drive, share, or needs-attention state change without using the manual refresh button | Within about 10 seconds, the Dashboard refreshes system health, drive summary, share summary, needs-attention content, and active-jobs content together so the cards and tables reflect the latest backend state |
| 1g | Dashboard summary drill-down preserves operational context | Open Dashboard as an operational user and activate a `Drive Summary` or `Shares Summary` entry with mouse or keyboard | ECUBE routes to the matching Drives or Shares page, and the destination filter control shows the corresponding drive state or share workflow bucket selected |
| 1ga | Shares workflow filter supports custody-pending drill-down | Open Dashboard with at least one completed share whose custody status is `PENDING_HANDOFF`, activate the `Custody Pending` summary entry, and inspect the Shares page | ECUBE routes to Shares with the `Custody Pending` workflow bucket preselected and only rows in that authoritative bucket remain visible |
| 1h | Dashboard needs-attention section highlights follow-up work | Seed failed, paused, and pending jobs plus completed jobs whose trusted custody state is both `PENDING_HANDOFF` and `HANDOFF_RECORDED` while the related drive remains mounted or `IN_USE`, then open Dashboard as a processor or manager | The Dashboard shows a `Needs Attention` section with rows labeled `Blocked`, `Waiting to Start`, `Waiting on Custody Closeout`, and `Ready to Eject`, and each `Job ID` value routes safely into the matching Job Detail page |
| 1i | Dashboard job rows show derived next-step guidance | Seed pending jobs that are both ready and still analyzing, running or pausing jobs, failed or paused jobs, a completed job with pending custody handoff, and a completed job with recorded custody handoff whose drive remains mounted or `IN_USE`, then open Dashboard as a processor or manager | The `Needs Attention` and active-jobs tables show a read-only `Next Step` column with guidance such as start, wait for analysis, monitor progress, review failed files, resume, review verification and handoff, or prepare eject based only on trusted job, custody, and drive mount state; running and pausing jobs stay visible with monitor guidance until the backend reaches a blocked or terminal state, and unknown or unsupported states fall back to opening Job Detail |
| 1j | Dashboard job rows surface trusted operational context | Seed an active job with a related mounted share, a destination drive assignment, and live transfer timing plus a blocked job with failed and timed-out file counts, then open Dashboard as a processor or manager | The Dashboard row metadata shows the trusted source path, destination drive label, failure counters for blocked work, and live-copy rate, time remaining, and estimated completion values when the backend fields are available; missing drive context falls back to `N/A` instead of inventing values, and the source-share label is visible only to `admin` and `manager` while `processor` sees the same redacted value used on Share Detail. At a mobile-width viewport, the Dashboard hides the `Project` column in `Needs Attention` and `Active Jobs`, keeps `Job ID` as the visible navigation control, preserves `Next Step` guidance, and reduces active-job progress to the percentage label with compact status icons. |
| 1k | Live copy rate excludes preparing time | Start or seed a job that remains in `PREPARING` for a measurable interval before entering `RUNNING`, then compare Job Detail and Dashboard after copy begins | `Duration` still reflects the full active lifecycle, while live `Copy rate`, `Time remaining`, and `Estimated completion` are calculated from the copy phase only and do not drop because of time spent in `PREPARING` |
...
| 3a | Folder browser updates source path during traversal | In Create Job, choose a mounted share, click `Browse folders`, navigate into a child directory, confirm the `Source path` field updates immediately, then use `..` to return toward the parent folder | The `Source path` field always matches the currently browsed root-relative folder, the `..` row only appears away from mount root, and the selected path remains inside the chosen mount |
...
| 1b | Edit dialog reuses the tabbed job shell | Open an editable pending job, click `Edit`, and compare the layout with `Create Job` | The edit dialog uses the same required-field marker, tab strip, footer legend, pinned header/footer scrolling behavior, and grouped content panels as `Create Job`, while `Run job immediately` remains create-only |
| 1c | Started jobs expose restricted Edit only | Open one `PAUSED` job, one `FAILED` job, one `RUNNING` job, and one `ARCHIVED` job in Job Detail | `RUNNING`, `PAUSED`, and `FAILED` jobs still show `Edit` and open on the `Workflow` tab for runtime tuning and reserved overflow-drive changes only. `ARCHIVED` jobs do not show `Edit`, while the remaining state-appropriate actions stay visible or disabled according to their own eligibility rules |
| 1d | Job Information panels present current user-entered and computed fields | Open Job Detail for a job created with notes, callback URL, explicit thread count, and at least one overflow assignment | `Job Information` shows `Job details`, `Source Information`, and `Destination Information` panels. `Job details` appears first and shows `Thread count`, `Callback URL`, and notes from job creation, while `Destination Information` shows the primary and overflow-drive audit fields, including serial number, vendor ID, and product ID when the trusted hardware metadata is available |
| 1e | Source totals show `N/A` before trusted counts are known | Open Job Detail for a pending job that has not completed startup analysis | `Source Information` shows `Discovered files` and `Estimated total bytes` as `N/A` until the backend supplies those values |
| 1f | Source totals update after analysis or active work determines them | Run `Analyze` for a pending job or open the same job after the backend has produced trusted source totals | `Source Information` updates `Discovered files` and `Estimated total bytes` to the trusted values without moving those fields to another panel |
...
| 4b | Prepare Eject appears only for mounted project-compatible related drives | Open a `COMPLETED` or `ARCHIVED` job as `admin` or `manager` where the related drive is mounted and bound to the same project, then repeat with the drive unmounted, assigned to a different project, and as a processor-only user. For the eligible case, exercise a drive that returns `EJECT_CONFIRM_REQUIRED` once before success. | Eligible jobs show `Prepare Eject`, confirmation uses the same ejection flow as Drive Detail, `EJECT_CONFIRM_REQUIRED` opens the incomplete-files confirmation prompt, confirm retries with override, and ineligible role or drive-state combinations hide the action |
...
| 15 | Files panel uses collapsed paged review flow | Open Job Detail for a job with more files than the configured page size | The Files panel starts collapsed, expands with `Show files`, shows `First` and `Last` shortcuts plus a numbered 10-page shortcut window, and preserves the current page when collapsed and reopened |
...
| 16 | Files table shows size beside the path without checksum listing | Expand the Files panel for a completed job and inspect the table headers and row actions | The table shows `Path`, `Size`, `Destination drive`, and `Status`; `Size` values use ECUBE byte formatting; the table does not show a `Checksum` column; and authorized users still have per-file hash inspection on the file path |
...
| 17a | Admin can tune exFAT format cluster size | Open `Configuration` as `admin`, change `exFAT Cluster Size`, save, then reopen `Configuration` | The selected cluster size persists after save and appears again on the next load so future exFAT formatting uses the updated value |
| 17b | Admin can tune drive format timeout | Open `Configuration` as `admin`, change `Drive Format Timeout (seconds)`, save, then reopen `Configuration` | The timeout persists after save and appears again on the next load so future drive format operations use the updated value |
| 17c | Admin can tune drive mount timeout | Open `Configuration` as `admin`, change `Drive Mount Timeout (seconds)`, save, then reopen `Configuration` | The timeout persists after save and appears again on the next load so future drive mount operations use the updated value |
| 17d | Admin can tune network mount timeout | Open `Configuration` as `admin`, change `Network Mount Timeout (seconds)`, save, then reopen `Configuration`; then validate or update a network mount | The timeout persists after save and appears again on the next load so future network mount, unmount, and mount-state verification workflows use the updated value |
| 17e | Admin can tune mount share discovery timeout | Open `Configuration` as `admin`, change `Mount Share Discovery Timeout (seconds)`, save, then reopen `Configuration`; then use `Browse` from the Add Share dialog | The timeout persists after save and appears again on the next load so future SMB share and NFS export discovery requests use the updated value |
| 17ea | Admin or manager can switch USB discovery runtime mode | Open `Configuration` as `admin` or `manager`, set `Auto USB Discovery Interval (seconds)` to `0`, save, verify udev event-driven updates, then set it to a positive value and verify timed polling behavior | The field persists after save, `0` selects event-driven udev monitoring mode, and positive values select timed polling mode without requiring a service restart |
| 17eb | VM passthrough hotplug stays live across USB discovery mode changes | In a VM with an actual USB passthrough device, open `Configuration` as `admin` or `manager`, set `Auto USB Discovery Interval (seconds)` to `0`, save, remove and reinsert the passthrough USB drive, then confirm the drive appears or refreshes on the Drives page without a manual rescan. Next set the interval to a positive value, save, remove and reinsert the same device again, and verify the drive state updates after the configured polling interval elapses | Event-driven mode reacts to the live passthrough hotplug without a manual rescan, polling mode reflects the same hardware change after the configured interval, and toggling between the two settings does not require a service restart or leave the drive list stale |
| 17f | Large-drive format stays in Drive Detail background flow | Start formatting a large exFAT-capable USB drive from Drive Detail and keep the page open longer than 30 seconds | The confirmation dialog closes promptly after submission, Drive Detail shows a formatting-in-progress message with conflicting drive actions disabled, and the page keeps refreshing until the drive either reports completion or shows a sanitized backend failure instead of a generic browser timeout |
| 17g | Drive Detail replaces a transient poll error with the final format failure | Start formatting a drive from Drive Detail, interrupt browser access to the API long enough for one refresh attempt to fail, then restore connectivity after the backend marks the format as failed | Drive Detail can show a transient network error during the outage, but once connectivity resumes and the failed drive state is fetched, the page replaces the generic network banner with the sanitized backend format failure |
| 17h | Format submission failure does not leave the drive stuck pending | In a test or fault-injection environment where background task scheduling is forced to fail, submit drive format from Drive Detail and then refresh the same drive | The request fails immediately with an ECUBE error, and a subsequent refresh shows no persistent formatting-in-progress state or disabled actions on the drive |
| 17i | Pending format blocks conflicting actions | Start formatting a drive from Drive Detail, then while the drive still reports `format_status = PENDING` attempt `Mount`, `Initialize`, `Prepare Eject`, and `Test Throughput` from the UI or direct API calls | Drive Detail keeps those actions disabled while formatting is pending, and any direct request that bypasses the UI guard returns `409` with a message telling the operator to wait for formatting to complete |
| 17j | Restart recovery clears orphaned pending format state | Start formatting a drive, restart the ECUBE backend before the background format task completes, then refresh Drive Detail after the service returns | The drive does not remain stuck in `PENDING`; after restart, Drive Detail shows a sanitized format failure that tells the operator the format was interrupted during restart and can be retried |
| 17k | Second worker startup does not fail an active format owned by another worker | In a multi-worker or rolling-restart environment, start formatting a drive, then bring up an additional ECUBE worker while the original worker is still running the background format task | The second worker skips the locked startup reconciliation pass instead of changing the active drive format to `FAILED`, and the original worker can still complete or fail the format based on the actual mkfs outcome |
| 18 | Job Detail summary shows related drive available space | Open Job Detail for a job with an assigned drive that has a persisted `available_bytes` value, then repeat with a job whose drive has no reading yet | The top summary shows destination, source path, callback URL, and the related drive available space; when no reading exists, the available-space field shows `-` rather than blocking the page |
| 18a | Job Detail shows notes and overflow assignment states | Open a job that was created with operator notes and one or more reserved or active overflow drives, then repeat while the job is `RUNNING` | Job Detail shows the saved notes when present, lists each overflow drive with its current `Reserved`, `Active`, or `Released` state, and the live copy summary shows the current run `Started at` timestamp |
| 18b | Job Detail completion summary keeps total duration separate from copy rate | Complete a job that spent measurable time in `PREPARING` before entering `RUNNING`, then open Job Detail | The completion summary `Duration` includes the full active lifecycle, and `Copy rate` reflects only the copy phase after `RUNNING` began |
...
| 0ab | Callback signing secret can be rotated from Admin UI without being displayed | Open Admin, confirm the signing secret status, enter a new Webhook Signing Secret, save, then refresh the page | Save succeeds, the secret input reloads empty, and the status indicates that a secret is configured |
| 0ac | Stored callback signing secret can be cleared from Admin UI | Open Admin with a configured signing secret, check Clear the stored signing secret, save, then refresh the page | Save succeeds, the secret input remains empty, and the status indicates that no signing secret is configured |
...
| 0c | HTTP job callback requires explicit confirmation | Create or edit a job, enter `http://example.com/webhook` in Webhook callback URL, save before selecting the insecure callback checkbox, then select the checkbox and save again | The first save is rejected with the testing-only confirmation message. After the checkbox is selected, save succeeds and Job Detail shows the saved HTTP callback URL |
| 0a | Callback URL can be cleared from Job Detail UI | Open a job with an existing `webhook.site` callback URL, choose Edit, clear Webhook callback URL, save, and refresh Job Detail | Job saves successfully and Job Detail shows no callback URL |
...
| 5ea | Prepare Eject emits the attached-drive callback | Create a job with a `webhook.site` HTTPS URL, complete or fail the job while its drive assignment remains unreleased, open the related drive, click `Prepare Eject`, then inspect the next request and the audit row | `webhook.site` receives `DRIVE_EJECT_PREPARED` with `drive_id`, `flush_ok`, and `unmount_ok` in `event_details`, and the corresponding eject audit row is linked to the same job |
...
|---|------------|-------|----------|
| 1 | Public login guidance is shown without password disclosure | Run `install.sh --demo`, then open the login page | The login screen displays the public-safe message plus username, label, and description fields. |
| 2 | Shared demo password supports managed self-serve access | Inspect the login panel and browser network response for the public auth config | The public payload includes the shared demo password, and the login form prefills the password input for demo sign-in; per-account passwords, internal paths, and private bootstrap-only fields remain hidden |
...
| 8 | Runtime default demo password follows Password Policy when unset | Enable demo mode with `DEMO_SHARED_PASSWORD` unset, verify demo sign-in works, change Password Policy settings, restart ECUBE, then retry demo sign-in using the updated effective password from demo configuration | The restarted service accepts a password that matches the updated policy for the demo accounts, and the login page continues to prefill the password field without displaying the raw value |
| 9 | Demo setup succeeds when the setup account is already the demo admin | Enable demo mode, complete setup through the wizard using the configured demo admin account, then inspect `/etc/passwd`, `/etc/group`, and the login page | Setup completes without the generic demo-seeding failure banner, the remaining configured demo users are created, and the login page continues to show demo guidance with managed password prefill behavior without requiring a restart or retry |
| 10 | Audit trail exists | Query audit logs for demo bootstrap and denied password actions | Audit entries exist for seed, reset, and authorization denial events |
| 11 | No sample content is staged automatically | Review the install root and demo metadata after `install.sh --demo` | Demo bootstrap affects demo accounts, login guidance, and role state only; it does not stage sample projects, files, USB state, or network mounts |
...
| 3 | Help does not break current workflow context | Open Help while on Drives, Shares, Jobs, or Audit, then close it | Closing Help returns to the same page and preserved workflow context |
...
| 17 | Docker Compose setup uses container PAM policy | Start `docker compose -f docker-compose.ecube.yml up -d --build`, open `/setup`, create a local admin password that satisfies the configured policy, and confirm the setup succeeds | Setup completes successfully, and `docker compose exec ecube-app grep -F 'password	requisite	pam_pwquality.so local_users_only' /etc/pam.d/common-password` shows the container-enforced password-quality stack |
| 18 | Docker Compose local password reset follows container policy | In a Compose deployment with demo mode or a local managed account enabled, trigger a password reset or demo-account reconciliation, then inspect `docker compose logs ecube-app` | The password operation succeeds without Cracklib dictionary-load errors, and logs do not show repeated `pam_chauthtok()` password-policy failures for policy-compliant values |
...
| 18 | Restart clears stale in-progress startup analysis when no valid cache remains | Trigger manual Analyze for a `PENDING` job, restart ECUBE before the scan completes, then reopen Job Detail | Job does not remain stuck in `ANALYZING`, startup-analysis state returns to `NOT_ANALYZED`, audit log contains `JOB_STARTUP_ANALYSIS_RECONCILED` with `reason: "analysis_interrupted_by_restart"`, and Analyze or Start is available again |
| 19 | Restart restores `READY` when the interrupted startup-analysis cache is still current | Trigger manual Analyze for a job with a stable source tree, restart ECUBE after the reusable snapshot has been persisted but before the manual analyze workflow fully completes, then reopen Job Detail | Job shows `startup_analysis_status: READY`, the previous file and byte totals are still available, audit log contains `JOB_STARTUP_ANALYSIS_RECONCILED` with `reason: "valid_cache_restored_after_restart"`, and Start can proceed without forcing a fresh scan |
...
| `lsusb` shows device but ECUBE doesn't | sysfs path may differ | Check `/sys/bus/usb/devices` is readable by `ecube` user. If running udev mode (`USB_DISCOVERY_INTERVAL=0`), verify monitor health logs; if running polling mode (`USB_DISCOVERY_INTERVAL>0`), wait one interval or trigger manual refresh |
