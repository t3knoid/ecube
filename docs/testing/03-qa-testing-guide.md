...
| 1c | Dashboard mounts summary reflects operator decision buckets | Seed mounts with no related job, a not-yet-started related job (`PENDING`), started or paused jobs (`RUNNING`, `PAUSING`, `PAUSED`, `VERIFYING`, or `FAILED`), terminal jobs whose custody is both pending and recorded, and mounts whose related-job context is unavailable, then open Dashboard as an operational user | The Dashboard shows a `Mounts Summary` card alongside `Drive Summary`, with `Unassigned`, `Assigned`, `In Progress`, `Completed`, and `Unavailable` counts that match the authoritative related-job and custody data returned by the backend; `Assigned` only counts not-yet-started jobs, completed or archived jobs stay `In Progress` until custody handoff is recorded, and unavailable related-job context is counted explicitly instead of being omitted |
