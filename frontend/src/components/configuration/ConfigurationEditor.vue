...
const props = defineProps({
  mode: {
    type: String,
    default: 'manager',
    validator: (value) => ['manager', 'admin'].includes(value),
  },
})
...
async function loadConfiguration() {
  ...
  try {
    const configurationResponse = isAdminMode.value
...
async function saveConfiguration() {
  ...
  const passwordPolicyPayload = isAdminMode.value && passwordPolicyAvailable.value ? buildPasswordPolicyPayload() : {}
