...
- `usb_drives` captures runtime device identity and current assignment/state.
  ...
  - `current_state` — FSM column (`DISCONNECTED`, `DISABLED`, `AVAILABLE`, `IN_USE`);
...
- `export_jobs` stores job-level lifecycle and throughput counters.
  ...
  - `callback_url` (nullable `String`) — optional HTTPS URL that receives POST callbacks for persisted job lifecycle events including creation, start, retry, pause, terminal completion/failure, manifest generation, chain-of-custody actions, archive, and reconciliation. Added in migration `0011`. Only `https://` URLs are accepted; HTTP is rejected at schema validation (422).
- Jobs in `RUNNING` or `VERIFYING` state cannot survive a service restart (worker processes are ephemeral). Startup reconciliation (§ 4.11) transitions these to `FAILED` with `completed_at` set and emits a `JOB_RECONCILED` webhook callback when callback delivery is configured.
...
- **Drive Reporting After Handoff:** A successful chain-of-custody handoff confirmation records the transfer event and refreshes the stored CoC snapshot, but the drive remains available to reporting and normal lifecycle rules based on its current state and project binding.
...
- `current_state` (Enum `DriveState: DISCONNECTED | DISABLED | AVAILABLE | IN_USE`, `native_enum=False`, default `AVAILABLE`)
  - Transitions: `DISCONNECTED ↔ DISABLED ↔ AVAILABLE` (discovery and port enablement), `AVAILABLE → IN_USE` (init), `IN_USE → AVAILABLE` (prepare-eject).
...
- `status` (Enum `JobStatus: PENDING | PREPARING | RUNNING | PAUSING | PAUSED | COMPLETED | FAILED | VERIFYING | ARCHIVED`, `native_enum=False`, default `PENDING`)
