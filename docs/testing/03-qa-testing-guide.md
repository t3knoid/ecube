...
| 4b | Prepare Eject appears only for mounted project-compatible related drives | Open a `COMPLETED` or `ARCHIVED` job as `admin` or `manager` where the related drive is mounted and bound to the same project, then repeat with the drive unmounted, assigned to a different project, and as a processor-only user | Eligible jobs show `Prepare Eject`, confirmation triggers the same ejection flow as Drive Detail, and ineligible role or drive-state combinations hide the action |
