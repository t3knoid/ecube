...
def test_post_install_demo_setup_creates_missing_local_database(tmp_path):
    ...
    assert "DEMO_LOGIN_MESSAGE=\n" in env_text
    assert "DEMO_SHARED_PASSWORD=Scene.9Pratt\n" in env_text
    assert "DEMO_DISABLE_PASSWORD_CHANGE=true\n" in env_text
    assert "DEMO_ACCOUNTS=[]\n" in env_text
...
def test_post_install_demo_setup_passes_shared_password_without_shell_reparsing(tmp_path):
    ...
    env_text = env_file.read_text(encoding="utf-8")
    assert f'DEMO_SHARED_PASSWORD={json.dumps(shared_password)}\n' in env_text
    assert bootstrap_log.read_text(encoding="utf-8").strip() == (
        f"--metadata-path {install_dir / 'demo-metadata.json'} seed --shared-password {shared_password}"
    )


def test_post_install_demo_setup_quotes_hashy_runtime_values_in_env(tmp_path):
    repo_root = Path(__file__).resolve().parent.parent
    install_dir = tmp_path / "install"
    env_file = install_dir / ".env"
    source_metadata = tmp_path / "demo-metadata.json"
    ...
    psql_log = tmp_path / "psql.log"
    bootstrap_log = tmp_path / "bootstrap.log"
    shared_password = "Scene #9 Pratt"

    source_metadata.write_text(
        json.dumps({"demo_config": {"login_message": "Use #1 demo access", "shared_password": "ignored-by-cli"}}),
        encoding="utf-8",
    )
    (install_dir / "venv" / "bin").mkdir(parents=True)
    (install_dir / "venv" / "bin" / "ecube-demo-bootstrap").write_text(
    ...
    assert result.returncode == 0, (
        f"post_install_demo_setup.sh failed with hashy runtime values:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
    )
    env_text = env_file.read_text(encoding="utf-8")
...
def test_post_install_demo_setup_allows_metadata_without_shared_password(tmp_path):
    ...
    assert (
        "DEMO_ACCOUNTS="
        + json.dumps([{"username": "demo_admin", "label": "", "description": ""}], separators=(",", ":"))
        + "\n"
    ) in env_text
    assert bootstrap_args.endswith(generated_password)


def test_post_install_demo_setup_generated_password_uses_active_policy_classes(tmp_path):
    repo_root = Path(__file__).resolve().parent.parent
    install_dir = tmp_path / "install"
    env_file = install_dir / ".env"
    source_metadata = tmp_path / "demo-metadata.json"
    fake_bin = tmp_path / "bin"
    policy_file = tmp_path / "pwquality.conf"
    state_file = tmp_path / "psql-state.txt"
    psql_log = tmp_path / "psql.log"
    bootstrap_log = tmp_path / "bootstrap.log"
    ...
    policy_file.write_text("minlen = 18\nminclass = 4\n", encoding="utf-8")
    (install_dir / "venv" / "bin").mkdir(parents=True)
    (install_dir / "venv" / "bin" / "ecube-demo-bootstrap").write_text(
        "#!/usr/bin/env bash\n"
        "set -euo pipefail\n"
    ...
    env = {
        **os.environ,
        ...
        "SOURCE_METADATA": str(source_metadata),
        "ENV_FILE": str(env_file),
        "PWQUALITY_CONF_PATH": str(policy_file),
        "BOOTSTRAP_LOG": str(bootstrap_log),
        "FAKE_PSQL_STATE": str(state_file),
        "FAKE_PSQL_LOG": str(psql_log),
    }
    ...
    assert result.returncode == 0, (
        f"post_install_demo_setup.sh failed with policy-driven generation:\nSTDOUT:\n{result.stdout}\nSTDERR:\n{result.stderr}"
    )
    generated_password = json.loads((install_dir / "demo-metadata.json").read_text(encoding="utf-8"))["demo_config"]["shared_password"]
