...
description: "Implement an ECUBE ticket with a root-cause fix, focused tests, verification, pattern reuse, and documentation review."
...
Treat the repository instructions and `.mdc` files as the single source of truth.  
Do not restate or loosen them.  
If one of them is missing, say so explicitly.

-------------------------------------------------------------------------------
## Pattern Discovery & DRY Enforcement (Mandatory)
-------------------------------------------------------------------------------

Before writing any code, you **must** perform a repository-wide pattern discovery step.

### 1. Search for existing patterns
Identify and inspect the closest existing implementation that solves a similar problem:

- existing services, models, schemas, repositories, and endpoints  
- existing Vue components, stores, composables, and UI patterns  
- existing error contracts (`R_*` schemas)  
- existing infrastructure wrappers  
- existing pagination, mount, job, drive, or audit patterns  
- existing logging, audit, and validation patterns  
- existing directory-browser or tree-view patterns  
- existing test patterns for similar behavior  

### 2. Enforce DRY
You must avoid DRY violations:

- do not duplicate logic, schemas, components, or patterns  
- do not introduce new abstractions if an equivalent one already exists  
- do not create new error contracts, new services, new endpoints, or new UI variants unless required  
- if you propose a new pattern, justify why reuse is not possible  
...
- existing design tokens, spacing, and UI treatments  
...
- **perform pattern discovery and reuse existing patterns/contracts**  
...
If the ticket appears to conflict with repository instructions, do not implement the conflicting behavior.  
...
3. **Perform pattern discovery and list the existing patterns/contracts you will reuse.**  
4. Read the relevant code, tests, and docs.  
5. If schema changes are required, resolve the current release migration from `project.version` and reuse it instead of creating a new Alembic revision file.  
6. Identify the root cause with evidence.  
7. Describe instruction compliance before coding, including trust boundaries, RBAC, auditability, service ownership, safe error handling, and migration-workflow compliance when applicable.  
8. Add or update focused tests.  
9. Implement the minimal root-cause fix **using the identified existing patterns**.  
10. Audit the changed behavior for required documentation, QA, and UI use-case updates using concrete evidence from the implementation or diff.  
...
14. State whether documentation is already aligned or which documents/comments needed updates and why.  
