...
| 20 | View the mounts list actions and browse controls | Raw remote and local mount paths are redacted in the table; the list exposes `Details` and `Browse`, does not expose `Edit` or `Remove`, and browse remains enabled only for mounted shares |
