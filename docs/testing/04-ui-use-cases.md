...
| UC-9.4 | Update copy and DB pool runtime settings (`copy_job_timeout`, `db_pool_size`, `db_pool_max_overflow`) | Admin | admin |
...
**UI Implication:** Admin-only `Configuration` page provides editable logging, copy job timeout, and DB pool fields, localized error handling, and a restart-required panel. Save applies supported changes immediately where possible and lists deferred settings that require service restart.
